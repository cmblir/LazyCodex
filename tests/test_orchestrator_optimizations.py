    ...
    def fake_exec(assignee, prompt, **kw):
        ...
        if "planner" in sysp:
            ...
            return SimpleNamespace(
                status="ok",
                output='{"plan":[{"assignee":"codex:sonnet","task":"step"}]}',
...
def test_plan_cache_distinguishes_text(orch, monkeypatch):
    ...
    def fake_exec(assignee, prompt, **kw):
        ...
        if "planner" in (kw.get("system_prompt") or "").lower():
            ...
            return SimpleNamespace(
                status="ok",
                output='{"plan":[{"assignee":"codex:sonnet","task":"x"}]}',
                error="", model="m", provider="p", tokens_total=1,
                duration_ms=1, cost_usd=0.0, raw={},
            )
        return SimpleNamespace(
...
def test_plan_cache_lru_evicts(orch, monkeypatch):
    ...
    for i in range(3):
        k = orch._plan_cache_key(f"t{i}", None, ["codex:sonnet"])
        orch._plan_cache_set(k, [{"assignee": "codex:sonnet", "task": str(i)}])
    k0 = orch._plan_cache_key("t0", None, ["codex:sonnet"])
    assert orch._plan_cache_get(k0) is None  # evicted
    k2 = orch._plan_cache_key("t2", None, ["codex:sonnet"])
