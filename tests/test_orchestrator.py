...
def orch(tmp_path, monkeypatch):
    monkeypatch.setenv("CODEX_DASHBOARD_ORCHESTRATOR",
                       str(tmp_path / "orch.json"))
    monkeypatch.setenv("CODEX_DASHBOARD_DB", str(tmp_path / "bus.db"))
...
def test_config_roundtrip(orch):
    ...
    r = orch.api_orch_config_save({
        "plannerAssignee": "codex:sonnet",
        "defaultAssignees": ["codex:sonnet", "openai:gpt-4.1"],
        "maxParallel": 3,
    })
    assert r["ok"]
    cfg = orch.load_config()
    assert cfg["plannerAssignee"] == "codex:sonnet"
...
def test_bind_unbind(orch):
    ...
    r2 = orch.api_orch_bind({"kind": "telegram", "chat": "-100",
                             "assignees": ["codex:opus"]})
    assert r2["ok"]
    assert len(r2["bindings"]) == 1
    assert r2["bindings"][0]["assignees"] == ["codex:opus"]
...
```json
{"plan":[
  {"assignee":"codex:sonnet","task":"summarise"},
  {"assignee":"openai:gpt-4.1","task":"counter-check"}
]}
```"""
    plan = orch._parse_plan(txt, ["codex:sonnet", "openai:gpt-4.1"])
    assert len(plan) == 2
    assert plan[0]["assignee"] == "codex:sonnet"
...
def test_parse_plan_substitutes_unknown_assignee(orch):
    ...
    assert plan == [{"assignee": "codex:sonnet", "task": "do thing"}]


def test_parse_plan_fallback_when_garbage(orch):
    plan = orch._parse_plan("not json at all", ["codex:sonnet"])
    assert plan == [{"assignee": "codex:sonnet", "task": "not json at all"}]
...
def test_dispatch_uses_stubbed_provider(orch, monkeypatch):
    ...
    def fake_exec(assignee, prompt, **kw):
        ...
        if "planner" in (kw.get("system_prompt") or "").lower():
            return SimpleNamespace(
                status="ok",
                output='{"plan":[{"assignee":"codex:sonnet","task":"step one"}]}',
    ...
    assert r["ok"]
    assert r["plan"][0]["assignee"] == "codex:sonnet"
