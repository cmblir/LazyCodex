"""Unit tests for server.mcp catalog + codex-mcp-list cache."""
...
def mcp_isolated(tmp_path, monkeypatch, isolated_home):
    ...
    # Pretend `codex` binary is missing so refresh is a no-op subprocess-wise.
...
class TestCodexMcpListCached:
    def test_returns_tuple_of_two_dicts(self, mcp_isolated):
        status, url = mcp_mod._codex_mcp_list_cached()
    ...
    def test_stale_cache_returns_cached_value_immediately(self, mcp_isolated, monkeypatch):
        ...
        status, _url = mcp_mod._codex_mcp_list_cached()
...
class TestRefreshBlockingMockedSubprocess:
    def test_refresh_with_no_codex_bin(self, mcp_isolated):
    ...
    def test_refresh_parses_subprocess_output(self, mcp_isolated, monkeypatch):
        monkeypatch.setattr(mcp_mod.shutil, "which", lambda name: "/usr/bin/codex")
