...
real ``~/.codex-dashboard-backups`` is never touched.
"""
...
def isolated_backup(tmp_path, monkeypatch, isolated_home):
    ...
    backups = home / ".codex-dashboard-backups"
    backups.mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr(backup_mod, "_HOME", home, raising=True)
    monkeypatch.setattr(backup_mod, "_DB_FILE", db, raising=True)
    json_files = [
        home / ".codex-dashboard-workflows.json",
        home / ".codex-dashboard-auto-resume.json",
        home / ".codex-dashboard-ai-providers.json",
        home / ".codex-dashboard-slack.json",
        home / ".codex-dashboard-prefs.json",
        home / ".codex-dashboard-hyper-agents.json",
        home / ".codex-code-router" / "config.json",
    ]
    ...
    monkeypatch.setattr(
        ...
        "_ARCHIVE_NAME_MAP",
        {
            ".codex-dashboard.db": db,
            ".codex-dashboard-workflows.json": json_files[0],
            ".codex-dashboard-auto-resume.json": json_files[1],
            ".codex-dashboard-ai-providers.json": json_files[2],
            ".codex-dashboard-slack.json": json_files[3],
            ".codex-dashboard-prefs.json": json_files[4],
            ".codex-dashboard-hyper-agents.json": json_files[5],
            "codex-code-router__config.json": json_files[6],
        },
        raising=True,
    )
...
class TestCreate:
    ...
    def test_filename_pattern(self, isolated_backup):
        _seed_data(isolated_backup)
        out = backup_mod.api_backup_create({"label": "test"})
        # lazycodex-YYYYMMDD-HHMMSS-test.tar.gz
        assert re.match(
            r"^lazycodex-\d{8}-\d{6}-test\.tar\.gz$", out["name"]
        ), out["name"]
    ...
    def test_no_label_still_works(self, isolated_backup):
        ...
        assert out["ok"] is True
        assert re.match(r"^lazycodex-\d{8}-\d{6}\.tar\.gz$", out["name"]), out["name"]
