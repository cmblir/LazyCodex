...
CCR_CONFIG_PATH is redirected into tmp_path so no real ~/.codex-code-router
...
def ccr_tmp_config(tmp_path, monkeypatch, isolated_home):
    ...
    cfg_dir = tmp_path / ".codex-code-router"
...
class TestStatus:
    def test_returns_expected_keys(self, ccr_tmp_config):
        ...
        for key in (
            "node_version", "ccr_installed", "codex_installed",
...
class TestAliasSnippet:
    ...
    def test_snippet_contains_zcodex_alias(self, ccr_tmp_config):
        out = ccr.api_ccr_alias_snippet({})
        assert "alias zcodex='ccr code'" in out["zshrc_snippet"]
        assert "alias zcodex='ccr code'" in out["bashrc_snippet"]
