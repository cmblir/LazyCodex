...
class TestProcessOneLifecycle:
    """End-to-end: idle+rate-limited jsonl -> spawn fake codex -> STATE_DONE -> auto-disable."""

    def _setup(self, tmp_path, monkeypatch, codex_exit: int, codex_stderr: str = ""):
        import os
        ...
        # Fake `codex` binary as a shell script
        ...
        if codex_exit == 0:
            ...
            fake_codex.write_text(
                f"#!/bin/sh\nprintf '%s' {codex_stderr!r} 1>&2\nexit {codex_exit}\n"
            )
        fake_codex.chmod(0o755)
        monkeypatch.setattr(ar, "_codex_bin", lambda: str(fake_codex))
    ...
    def test_clean_exit_marks_done_and_disables(self, tmp_path, monkeypatch):
        ar, sid = self._setup(tmp_path, monkeypatch, codex_exit=0)
        ar._process_one(sid)
        store = ar._load_all()
        e = store[sid]
    ...
    def test_rate_limit_exit_schedules_retry_keeps_enabled(self, tmp_path, monkeypatch):
        ar, sid = self._setup(
            tmp_path, monkeypatch, codex_exit=1,
            codex_stderr="HTTP 429 Too Many Requests",
        )
        ar._process_one(sid)
        store = ar._load_all()
        e = store[sid]
    ...
    def _stub_bind_path(self, ar, sid, monkeypatch):
        ...
        a real ~/.codex/projects/<id>/<sid>.jsonl."""
        ...
        # _codex_bin already stubbed in _setup; don't override.
    ...
    def test_deadline_in_past_exhausts_immediately(self, tmp_path, monkeypatch):
        ...
        # tick must flip state to exhausted without spawning codex.
        ar, sid = self._setup(
            tmp_path, monkeypatch, codex_exit=1,
            codex_stderr="HTTP 429 Too Many Requests",
        )
    ...
    def test_auth_expired_disables_permanently(self, tmp_path, monkeypatch):
        ar, sid = self._setup(
            tmp_path, monkeypatch, codex_exit=1,
            codex_stderr="Unauthorized: please run /login",
        )
        ar._process_one(sid)
        store = ar._load_all()
        e = store[sid]
