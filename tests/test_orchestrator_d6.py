...
def test_step_succeeds_with_primary(orch, monkeypatch):
    ...
    out = orch._execute_step("rid", 0, "codex:opus", "task",
                             fallback_chain=["codex:sonnet"])
    assert out["ok"] and out["assignee"] == "codex:opus"
...
def test_step_falls_over_when_primary_errors(orch, monkeypatch):
    ...
    out = orch._execute_step("rid", 0, "codex:opus", "task",
                             fallback_chain=["codex:sonnet", "openai:gpt-4.1"])
    assert seen == ["codex:opus", "codex:sonnet"]
    assert out["assignee"] == "codex:sonnet"
...
def test_step_dedupes_chain(orch, monkeypatch):
    ...
    orch._execute_step("rid", 0, "codex:opus", "task",
                       fallback_chain=["codex:opus", "codex:sonnet"])
    assert seen == ["codex:opus"]
...
def test_sanitize_keeps_fallback_and_budget(orch):
    b = orch._sanitize_binding({
        ...
        "fallbackChain": ["codex:sonnet", "openai:gpt-4.1", " "],
        "budgetUsdPerDay": "2.5",
    })
    assert b["kind"] == "discord"
    assert b["fallbackChain"] == ["codex:sonnet", "openai:gpt-4.1"]
