...
page.on('response', async (resp) => {
  try {
    ...
    if (size > 50_000) {
      resourceSummary.push({ url: url.replace(`http://127.0.0.1:${process.env.PORT || 19500}`, ''), size });
    }
  } catch (e) {}
});

const t0 = Date.now();
await page.goto(`http://127.0.0.1:${process.env.PORT || 19500}/#/workflows`, { waitUntil: 'networkidle' });
