...
NEW_EN: dict[str, str] = {
    ...
    "이 세션이 토큰/레이트 한도로 멈추면 백그라운드 워커가 codex --resume 으로 자동 재시도합니다 (셸 while-loop 동등).":
        "If this session is halted by a token / rate limit, a background worker auto-retries it with `codex --resume` (equivalent to the shell while-loop).",
    ...
    "실행 중 (codex --resume)": "Retrying (codex --resume)",
...
NEW_ZH: dict[str, str] = {
    ...
    "이 세션이 토큰/레이트 한도로 멈추면 백그라운드 워커가 codex --resume 으로 자동 재시도합니다 (셸 while-loop 동등).":
        "如果此会话因令牌 / 速率限制而停止,后台工作进程会用 `codex --resume` 自动重试(相当于 shell while 循环)。",
    ...
    "실행 중 (codex --resume)": "重试中(codex --resume)",
