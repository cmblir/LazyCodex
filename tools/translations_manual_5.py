...
NEW_EN = {
    ...
    "~/.codex/skills/<id>/SKILL.md 로 자신만의 스킬 추가. 13개 = 만점.":
        "Add your own skills via ~/.codex/skills/<id>/SKILL.md. 13 = full score.",
    "~/.codex/agents/<name>.md 로 자신만의 서브에이전트 추가.":
        "Add your own sub-agents via ~/.codex/agents/<name>.md.",
    ...
    "기본 모델 override (예: codex-opus-4-7)": "Default model override (e.g. codex-opus-4-7)",
    "Haiku 등 작은 보조 모델 지정": "Small helper model (e.g. Haiku)",
    "~/.codex 위치 override": "Override ~/.codex location",
    ...
    "Codex CLI 는 현재 VS Code / JetBrains IDE 에 bridge 방식으로 연결됩니다. 연결된 IDE 는 세션 metadata 의 entrypoint/terminal 필드로 식별.":
        "Codex CLI connects to VS Code / JetBrains IDEs via a bridge. Connected IDEs are identified by the session metadata's entrypoint/terminal fields.",
    # NOTE: keys must use the ORIGINAL Korean tokens (before short-key substitution)
    "토큰은 세션 DB (~/.codex/projects/*/*.jsonl 에서 파싱) 기반":
        "Tokens are parsed from the session DB (~/.codex/projects/*/*.jsonl)",
...
NEW_ZH = {
    ...
    "하나만 추가해도": "仅添加一个即可获得",
    "~/.codex/skills/<id>/SKILL.md 로 자신만의 스킬 추가. 13개 = 만점.":
        "通过 ~/.codex/skills/<id>/SKILL.md 添加自己的技能。13 = 满分。",
    "~/.codex/agents/<name>.md 로 자신만의 서브에이전트 추가.":
        "通过 ~/.codex/agents/<name>.md 添加自己的子代理。",
    "로 자신만의": "添加自己的",
    "자신만의 스킬 추가": "自己的技能",
    ...
    "기본 모델 override (예: codex-opus-4-7)": "默认模型覆盖（例：codex-opus-4-7）",
    "Haiku 등 작은 보조 모델 지정": "指定 Haiku 等小型辅助模型",
    "~/.codex 위치 override": "覆盖 ~/.codex 位置",
    ...
    "Codex CLI 는 현재 VS Code / JetBrains IDE 에 bridge 방식으로 연결됩니다. 연결된 IDE 는 세션 metadata 의 entrypoint/terminal 필드로 식별.":
        "Codex CLI 通过 bridge 方式连接 VS Code / JetBrains IDE。已连接的 IDE 通过会话元数据的 entrypoint/terminal 字段识别。",
    "토큰은 세션 DB (~/.codex/projects/*/*.jsonl 에서 파싱) 기반":
        "令牌来自会话 DB（解析自 ~/.codex/projects/*/*.jsonl）",
    "비용은 2025 공식 요금표 기반 추정":
        "费用基于 2025 官方价目预估",
    "에서 파싱": "解析自",
    "공식 요금표": "官方价目",
