...
NEW_EN = {
    ...
    # Managed Agents
    "Codex 를 완전 관리형 에이전트 하네스로 실행. 샌드박스 + 내장 도구 + SSE 스트리밍.":
        "Run Codex as a fully managed agent harness. Sandbox + built-in tools + SSE streaming.",

    # ant CLI
    "Codex API 커맨드라인 클라이언트. Codex CLI 네이티브 통합 + YAML 리소스 버전 관리.":
        "Codex API command-line client. Codex CLI native integration + YAML resource versioning.",
    ...
    # Agent Skills
    "스킬(지시+스크립트+리소스 묶음) 을 Codex 가 동적으로 로드. PowerPoint/Excel/Word/PDF 기본 제공.":
        "Codex dynamically loads skills (instructions + scripts + resources bundles). Built-in PowerPoint / Excel / Word / PDF.",

    # Codex Mythos
    "Codex Mythos (보안)": "Codex Mythos (Security)",
...
NEW_ZH = {
    ...
    "Codex 를 완전 관리형 에이전트 하네스로 실행. 샌드박스 + 내장 도구 + SSE 스트리밍.":
        "将 Codex 作为完全托管的代理框架运行。沙箱 + 内置工具 + SSE 流式传输。",

    "Codex API 커맨드라인 클라이언트. Codex CLI 네이티브 통합 + YAML 리소스 버전 관리.":
        "Codex API 命令行客户端。Codex CLI 原生集成 + YAML 资源版本管理。",
    ...
    "스킬(지시+스크립트+리소스 묶음) 을 Codex 가 동적으로 로드. PowerPoint/Excel/Word/PDF 기본 제공.":
        "Codex 动态加载技能（指令+脚本+资源集合）。默认提供 PowerPoint / Excel / Word / PDF。",

    "Codex Mythos (보안)": "Codex Mythos（安全）",
    "방어 보안 특화 언어모델. 초대제 research preview (Project Glasswing).":
