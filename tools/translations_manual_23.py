...
NEW_EN: dict[str, str] = {
    ...
    "Codex CLI CLI 세션의 PID·RSS·CPU·idle 시간 + 터미널 포커스 / SIGTERM":
        "Codex CLI CLI session PID/RSS/CPU/idle time + terminal focus / SIGTERM",
    "vm_stat 기반 시스템 메모리 + 상위 30 프로세스 + idle Codex CLI 일괄 종료":
        "System memory via vm_stat + top 30 processes + bulk-kill idle Codex CLI",

    # View subtitles
    "TCP/UDP listening 소켓 모니터링": "TCP/UDP listening socket monitor",
    "Codex CLI CLI 세션 모니터링": "Codex CLI CLI session monitor",
    ...
    "idle Codex CLI 정리": "Clean up idle Codex CLI",
    "idle Codex CLI 일괄 종료": "Bulk kill idle Codex CLI",
    "idle 600초 이상 세션": "Sessions idle ≥ 600s",

    # Confirm / toast / error
    "정말 이 프로세스를 종료하시겠습니까?": "Really kill this process?",
    "idle 상태인 모든 Codex CLI 세션을 종료하시겠습니까?":
        "Kill all idle Codex CLI sessions?",
...
NEW_ZH: dict[str, str] = {
    ...
    "Codex CLI CLI 세션의 PID·RSS·CPU·idle 시간 + 터미널 포커스 / SIGTERM":
        "Codex CLI CLI 会话的 PID/RSS/CPU/空闲时间 + 聚焦终端 / SIGTERM",
    "vm_stat 기반 시스템 메모리 + 상위 30 프로세스 + idle Codex CLI 일괄 종료":
        "基于 vm_stat 的系统内存 + 前 30 进程 + 批量终止空闲 Codex CLI",

    # View subtitles
    "TCP/UDP listening 소켓 모니터링": "TCP/UDP 监听套接字监控",
    "Codex CLI CLI 세션 모니터링": "Codex CLI CLI 会话监控",
    ...
    "종료": "终止",
    "터미널 열기": "打开终端",
    "idle Codex CLI 정리": "清理空闲 Codex CLI",
    "idle Codex CLI 일괄 종료": "批量终止空闲 Codex CLI",
    "idle 600초 이상 세션": "空闲 ≥ 600 秒的会话",

    # Confirm / toast / error
    "정말 이 프로세스를 종료하시겠습니까?": "确定要终止该进程吗？",
    "idle 상태인 모든 Codex CLI 세션을 종료하시겠습니까?":
        "确定要终止所有空闲 Codex CLI 会话吗？",
    "프로세스를 종료했습니다": "进程已终止",
    "종료 실패": "终止失败",
    "터미널을 활성화했습니다": "终端已激活",
    "터미널 열기 실패": "打开终端失败",
