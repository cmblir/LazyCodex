...
NEW_EN: dict[str, str] = {
    ...
    "대상 Codex 세션을 골라서 Auto-Resume 워커를 붙입니다. 토큰/레이트 한도 발생 시 자동으로 codex --resume 으로 재시도합니다.":
        "Pick a target Codex session to attach an Auto-Resume worker. "
        "On token/rate-limit failure the worker auto-retries via `codex --resume`.",
    "실행 중인 세션 선택":              "Pick a running session",
    "직접 입력":                        "Enter manually",
    "실행 중인 Codex CLI 세션이 감지되지 않습니다. session UUID 와 cwd 를 직접 입력하세요.":
        "No running Codex CLI session detected. Enter the session UUID and "
        "cwd manually.",
...
NEW_ZH: dict[str, str] = {
    ...
    "새 바인딩 추가":                   "新增绑定",
    "대상 Codex 세션을 골라서 Auto-Resume 워커를 붙입니다. 토큰/레이트 한도 발생 시 자동으로 codex --resume 으로 재시도합니다.":
        "选择目标 Codex 会话以挂载 Auto-Resume 工作进程。当 token / rate limit 触发时,工作进程会自动通过 `codex --resume` 重试。",
    "실행 중인 세션 선택":              "选择运行中的会话",
    "직접 입력":                        "手动输入",
    "실행 중인 Codex CLI 세션이 감지되지 않습니다. session UUID 와 cwd 를 직접 입력하세요.":
        "未检测到运行中的 Codex CLI 会话。请手动输入 session UUID 和 cwd。",
    "Session UUID":                     "Session UUID",
