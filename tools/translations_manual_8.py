...
NEW_EN = {
    ...
    # ── Onboarding UI ──
    "🚀 시작하기": "🚀 Getting started",
    "처음 사용자를 위한 단계별 체크리스트 — 현재 ~/.codex 상태를 실시간으로 감지합니다.":
        "Step-by-step checklist for new users — continuously detects your ~/.codex state.",
    "셋업 진행률": "Setup progress",
    "완료 🎉 — 이제 AI 종합 평가 탭에서 품질 점수를 받아보세요.":
        "Done 🎉 — head over to the AI Evaluation tab to get your quality score.",
    "남은 단계를 하나씩 체크하면 Codex CLI 를 더 안전하고 생산적으로 쓸 수 있습니다.":
        "Ticking off each remaining step will make Codex CLI safer and more productive.",
    ...
    # ── Guide Hub UI labels ──
    "📚 가이드 & 툴": "📚 Guide & Tools",
    "세 리소스(everything-codex-code · codex-code-best-practice · 위키독스 한국어 가이드) 를 한 곳에서.":
        "Three resources (everything-codex-code · codex-code-best-practice · Korean WikiDocs guide) in one place.",
    "🧰 유용한 툴": "🧰 Useful tools",
    ...
    "everything-codex-code 플러그인 설치 → 실전 기능 확보":
        "Install the everything-codex-code plugin for production-grade features",
    ...
    "멀티 플랫폼 클로드 코드 최적화 팩 (48 agents · 183 skills · 79 commands)":
        "Multi-platform Codex CLI optimization pack (48 agents · 183 skills · 79 commands)",
    ...
    "Codex CLI / Cursor / Codex / OpenCode 크로스 플랫폼":
        "Cross-platform: Codex CLI / Cursor / Codex / OpenCode",
    ...
    "Cross-Model: Codex CLI + Codex 조합":
        "Cross-model: Codex CLI + Codex combo",
    ...
    "AGENTS.md 작업 기억 · 첫 실전 프로젝트":
        "AGENTS.md working memory · First production project",
    ...
    "Codex CLI 에서 비용·지연·품질 모두 개선하는 핵심 스위치 5개.":
        "Five core switches in Codex CLI that improve cost, latency, and quality.",
    ...
    "Boris Cherny(Codex CLI 창시자)가 반복 강조하는 운용 팁.":
        "Operational tips Boris Cherny (Codex CLI creator) repeats.",
    "Plan 먼저": "Plan first",
    ...
    "Codex CLI 사용법과 명령어 전체 목록":
        "How to use Codex CLI and the full command list",
    ...
    "현재 레포용 AGENTS.md 를 자동 초기화":
        "Initialize AGENTS.md for the current repo",
    "PR 코드리뷰 — 체크리스트 기반":
    ...
    "Codex CLI 환경 진단 (auth/MCP/hook 상태)":
        "Diagnose your Codex CLI environment (auth/MCP/hook)",
    "현재 계정 로그아웃": "Log out of the current account",
    "Codex 계정 로그인": "Log in to a Codex account",
    ...
    "Codex CLI CLI 설치": "Install the Codex CLI CLI",
    "`~/.codex` 디렉토리가 존재해야 모든 기능이 동작합니다.":
        "Every feature requires the `~/.codex` directory to exist.",
    "npm i -g @anthropic-ai/codex-code 또는 공식 설치 스크립트.":
        "Run `npm i -g @anthropic-ai/codex-code` or use the official installer.",
    "전역 AGENTS.md 작성": "Write a global AGENTS.md",
    ...
    "AGENTS.md 탭에서 편집 · `/init` 으로 자동 생성 가능.":
        "Edit from the AGENTS.md tab · or bootstrap with `/init`.",
    ...
    "스킬은 클로드가 자동으로 적재하는 지식 모듈입니다.":
        "Skills are knowledge modules Codex loads on demand.",
    "Everything Codex CLI 설치 시 183개가 한 번에 추가됩니다.":
        "Installing Everything Codex CLI adds 183 at once.",
    ...
    "Everything Codex CLI 같은 번들로 한 번에 셋업.":
        "Bundles like Everything Codex CLI set you up in one shot.",
    "플러그인 탭 → 마켓 추가 → everything-codex-code 설치.":
        "Plugins tab → add market → install everything-codex-code.",
...
NEW_ZH = {
    ...
    "🚀 시작하기": "🚀 入门指南",
    "처음 사용자를 위한 단계별 체크리스트 — 현재 ~/.codex 상태를 실시간으로 감지합니다.":
        "面向新用户的分步清单 — 实时检测 ~/.codex 状态。",
    "셋업 진행률": "配置进度",
    "완료 🎉 — 이제 AI 종합 평가 탭에서 품질 점수를 받아보세요.":
        "已完成 🎉 — 现在可在「AI 综合评估」选项卡获取质量评分。",
    "남은 단계를 하나씩 체크하면 Codex CLI 를 더 안전하고 생산적으로 쓸 수 있습니다.":
        "逐项完成剩余步骤，让 Codex CLI 更安全、更高效。",
    "모든 항목 체크 후엔 'AI 종합 평가' 탭에서 0~100 점수를 받아보세요.":
    ...
    "📚 가이드 & 툴": "📚 指南与工具",
    "세 리소스(everything-codex-code · codex-code-best-practice · 위키독스 한국어 가이드) 를 한 곳에서.":
        "三大资源（everything-codex-code · codex-code-best-practice · 韩文 WikiDocs 指南）一站聚合。",
    "🧰 유용한 툴": "🧰 实用工具",
    "🔗 외부 가이드": "🔗 外部指南",
    "💡 베스트 프랙티스": "💡 最佳实践",
    "⌨️ 치트시트": "⌨️ 速查表",
    ...
    "everything-codex-code 플러그인 설치 → 실전 기능 확보":
        "安装 everything-codex-code 插件 → 获取生产级能力",
    "시작하기 탭으로 돌아와 체크리스트 완료":
        "返回「入门指南」并完成清单",
    "/ 내장 슬래시 명령어": "/ 内置斜杠命令",
    "내장 슬래시 명령어": "内置斜杠命令",
    ...
    # ── server/guide.py — Toolkit subtitles / meta ──
    "멀티 플랫폼 클로드 코드 최적화 팩 (48 agents · 183 skills · 79 commands)":
        "多平台 Codex CLI 优化套件（48 个 agents · 183 个 skills · 79 个 commands）",
    "47k ⭐ · 82 가지 팁 모음 + 창시자(Boris Cherny) 워크플로":
    ...
    # ── Toolkit highlights ──
    "Codex CLI / Cursor / Codex / OpenCode 크로스 플랫폼":
        "跨平台：Codex CLI / Cursor / Codex / OpenCode",
    "AgentShield 보안 감사 (1282 tests, 102 정적 룰)":
    ...
    "Codex CLI 에서 비용·지연·품질 모두 개선하는 핵심 스위치 5개.":
        "在 Codex CLI 中同时改善成本、延迟与质量的 5 个核心开关。",
    "모델 라우팅": "模型路由",
    "일상은 Sonnet 4.6, 무거운 분석만 Opus 4.7 — 평균 60% 비용 절감.":
    ...
    "Boris Cherny(Codex CLI 창시자)가 반복 강조하는 운용 팁.":
        "Boris Cherny（Codex CLI 创始人）反复强调的操作要点。",
    "Plan 먼저": "先做计划",
    "모든 비자명한 작업은 Plan Mode 로. 코드 생성 전 사용자 승인을 받는다.":
    ...
    "테마·모델 등 단순 설정 빠르게 전환":
        "快速切换主题/模型等简单设置",
    "Codex CLI 환경 진단 (auth/MCP/hook 상태)":
        "诊断 Codex CLI 环境（auth/MCP/hook 状态）",
    "현재 계정 로그아웃": "注销当前账户",
    "Codex 계정 로그인": "登录 Codex 账户",
    ...
    # ── Onboarding steps ──
    "Codex CLI CLI 설치": "安装 Codex CLI CLI",
    "`~/.codex` 디렉토리가 존재해야 모든 기능이 동작합니다.":
        "所有功能都依赖 `~/.codex` 目录存在。",
    "npm i -g @anthropic-ai/codex-code 또는 공식 설치 스크립트.":
        "运行 `npm i -g @anthropic-ai/codex-code` 或使用官方安装脚本。",
    "전역 AGENTS.md 작성": "编写全局 AGENTS.md",
    "모든 세션에 로드되는 개인/팀 규약. 100자 이상 작성을 권장합니다.":
        "在所有会话中加载的个人/团队约定。建议不少于 100 个字符。",
    "AGENTS.md 탭에서 편집 · `/init` 으로 자동 생성 가능.":
        "在 AGENTS.md 标签中编辑 · 也可用 `/init` 自动生成。",
    ...
    "스킬은 클로드가 자동으로 적재하는 지식 모듈입니다.":
        "Skills 是 Codex 按需加载的知识模块。",
    "Everything Codex CLI 설치 시 183개가 한 번에 추가됩니다.":
        "安装 Everything Codex CLI 会一次性加入 183 个。",
    ...
    "Everything Codex CLI 같은 번들로 한 번에 셋업.":
        "使用像 Everything Codex CLI 这样的合集一次完成配置。",
    "플러그인 탭 → 마켓 추가 → everything-codex-code 설치.":
        "Plugins 标签 → 添加市场 → 安装 everything-codex-code。",
    "출력 스타일 선택": "选择输出样式",
    "답변 톤/포맷을 프로젝트 성격에 맞게 고정.":
        "根据项目性质固定回复的语气 / 格式。",
    "출력 스타일 탭에서 커스텀 스타일을 만들거나 기본값 확인.":
