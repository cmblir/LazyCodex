...
# v2.37.0 — Auto-Resume: inject a retry loop into a live Codex CLI session
try:
...
# v2.42.0 — Computer Use / Memory / Advisor labs + Codex CLI Routines
try:
...
# v2.45.0 — Codex CLI Router (CCR / zcodex) setup wizard
try:
...
MANUAL_EN: dict[str, str] = {
    ...
    "Codex CLI 미설치": "Codex CLI not installed",
    "Codex CLI 설치됨": "Codex CLI installed",
    "Codex CLI 가 읽는 환경변수들. settings.json.env 값과 현재 프로세스 실제 값 비교": "Environment variables Codex CLI reads — comparing settings.json.env against the actual running-process values",
    "Codex CLI 내부 추적기 ·": "Codex CLI internal tracker ·",
    "Codex CLI 에서": "In Codex CLI",
    "Codex CLI 와 연결된 터미널/IDE 상태": "Terminal/IDE status connected to Codex CLI",
    "Codex Control Center — 최적화 대시보드": "Codex Control Center — Optimization Dashboard",
    "Codex 가 최근 발표한 신기능을 찾아서 신기능 탭에 추가": "Fetch Codex's latest announcements and add them to the New Features tab",
    "Codex 계정에 로그인되지 않았습니다.": "Not logged into Codex account.",
    ...
    "~/.codex/scheduled-tasks/ 에 정의된 cron 스킬": "cron skills defined in ~/.codex/scheduled-tasks/",
    "· 예상": "· estimated",
    ...
    "➕ AGENTS.md 에 추가": "➕ Append to AGENTS.md",
    ...
    "검색… ⌘K": "Search… ⌘K",
    "권한": "Permissions",
    "는 Codex CLI 가 자동으로 발견한 모든 프로젝트 루트를 기록합니다. 우리가": "records every project root Codex CLI auto-discovers. Unlike what we see via",
    "다크": "Dark",
    "또는": "or",
    "또는 Codex CLI 재시작으로 동기화.": "or restart Codex CLI to sync.",
    ...
    "🌿 Codex CLI 환경 변수 카탈로그": "🌿 Codex CLI environment variable catalog",
    ...
    "💻 활성 Codex CLI 세션 (": "💻 Active Codex CLI sessions (",
...
MANUAL_ZH: dict[str, str] = {
    ...
    "Codex CLI 미설치": "Codex CLI 未安装",
    "Codex CLI 설치됨": "Codex CLI 已安装",
    "Codex CLI 가 읽는 환경변수들. settings.json.env 값과 현재 프로세스 실제 값 비교": "Codex CLI 读取的环境变量 — 对比 settings.json.env 与实际运行进程的值",
    "Codex CLI 내부 추적기 ·": "Codex CLI 内部追踪器 ·",
    "Codex CLI 에서": "在 Codex CLI 中",
    "Codex CLI 와 연결된 터미널/IDE 상태": "与 Codex CLI 连接的终端/IDE 状态",
    "Codex Control Center — 최적화 대시보드": "Codex 控制中心 — 优化仪表板",
    "Codex 가 최근 발표한 신기능을 찾아서 신기능 탭에 추가": "获取 Codex 最新发布的新功能并添加到新功能标签",
    "Codex 계정에 로그인되지 않았습니다.": "未登录 Codex 账户。",
    ...
    "~/.codex/scheduled-tasks/ 에 정의된 cron 스킬": "~/.codex/scheduled-tasks/ 中定义的 cron 技能",
    "· 예상": "· 预估",
    ...
    "검색… ⌘K": "搜索… ⌘K",
    "권한": "权限",
    "는 Codex CLI 가 자동으로 발견한 모든 프로젝트 루트를 기록합니다. 우리가": "记录 Codex CLI 自动发现的所有项目根目录。与我们通过",
    "다크": "暗色",
    "또는": "或",
    "또는 Codex CLI 재시작으로 동기화.": "或重启 Codex CLI 以同步。",
    ...
    "＋ 새 마켓플레이스 추가": "＋ 添加市场",
    "＋ 이 프로젝트에 설치": "＋ 安装到此项目",
    "🌿 Codex CLI 환경 변수 카탈로그": "🌿 Codex CLI 环境变量目录",
    ...
    "💾 이 내용으로 저장": "💾 以此内容保存",
    "💻 활성 Codex CLI 세션 (": "💻 活跃 Codex CLI 会话 (",
    "📁 최근 텔레메트리 파일": "📁 最近遥测文件",
    "📊 상위 이벤트 타입": "📊 顶部事件类型",
    "📋 사용 가능 모델": "📋 可用模型",
    "📌 수동 단계": "📌 手动步骤",
