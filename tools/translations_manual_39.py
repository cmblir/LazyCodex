...
NEW_EN: dict[str, str] = {
    ...
    "Slack/Telegram 채널 멘션 → Codex 플래너 → 멀티 모델 병렬 분담 → 채널 회신. 에이전트 간 라이브 보고는 Agent Bus 탭/SSE 로 연결.":
        "Slack/Telegram channel mention → Codex planner → multi-model parallel split → channel reply. Live inter-agent reports via Agent Bus tab / SSE.",
    "Slack/Telegram/Discord 채널 멘션 → Codex 플래너 → 멀티 모델 병렬 분담 → 채널 회신. 채널별 fallback + 일일 예산 cap.":
        "Slack/Telegram/Discord channel mention → Codex planner → multi-model parallel split → channel reply. Per-channel fallback + daily budget cap.",
    ...
    "~/.codex/scheduled-tasks/ 목록 + Auto-Resume 활성 워커.":
        "~/.codex/scheduled-tasks/ list + active Auto-Resume workers.",
    "~/.codex/scheduled-tasks/*.yaml CRUD + 즉시 실행 — Codex CLI Routines":
        "~/.codex/scheduled-tasks/*.yaml CRUD + immediate execution — Codex CLI Routines",
    "Codex CLI를 GLM/Z.AI/DeepSeek 등 다른 LLM으로 라우팅 + zcodex 별칭 안내":
        "Route Codex CLI to other LLMs such as GLM/Z.AI/DeepSeek + zcodex alias guide",
    "대상 Codex 세션 UUID(또는 입력 문자열에서 추출)에 Auto-Resume 워커를 바인딩/해제":
        "Bind/unbind an Auto-Resume worker to a Codex session UUID (or extracted from the input string)",
    "워크플로우는 즉시 다음 노드로 진행되고, 워커는 백그라운드에서 토큰/레이트 한도 발생 시 codex --resume 으로 자동 재시도.":
        "The workflow advances immediately to the next node; the worker retries in the background via codex --resume when hitting token/rate limits.",
    ...
    # Path fragments
    "/ 하위경로는":            "/ sub-path",
    "/AGENTS.md 로 기록됩니다.": "is recorded in /AGENTS.md.",
}
...
NEW_ZH: dict[str, str] = {
    ...
    "Slack/Telegram 채널 멘션 → Codex 플래너 → 멀티 모델 병렬 분담 → 채널 회신. 에이전트 간 라이브 보고는 Agent Bus 탭/SSE 로 연결.":
        "Slack/Telegram 频道提及 → Codex 规划器 → 多模型并行分工 → 频道回复。跨 Agent 实时报告通过 Agent Bus 标签/SSE 连接。",
    "Slack/Telegram/Discord 채널 멘션 → Codex 플래너 → 멀티 모델 병렬 분담 → 채널 회신. 채널별 fallback + 일일 예산 cap.":
        "Slack/Telegram/Discord 频道提及 → Codex 规划器 → 多模型并行分工 → 频道回复。按频道 fallback + 每日预算上限。",
    "Geoffrey Huntley Ralph Wiggum 패턴 — 같은 PROMPT.md를 4중 안전장치 안에서 반복. 프로젝트 추천기 + 라이브 SSE + CLI.":
    ...
    "~/.codex/scheduled-tasks/ 목록 + Auto-Resume 활성 워커.":
        "~/.codex/scheduled-tasks/ 列表 + 活跃 Auto-Resume 工作器。",
    "~/.codex/scheduled-tasks/*.yaml CRUD + 즉시 실행 — Codex CLI Routines":
        "~/.codex/scheduled-tasks/*.yaml CRUD + 立即执行 — Codex CLI Routines",
    "Codex CLI를 GLM/Z.AI/DeepSeek 등 다른 LLM으로 라우팅 + zcodex 별칭 안내":
        "将 Codex CLI 路由到 GLM/Z.AI/DeepSeek 等其他 LLM + zcodex 别名指南",
    "대상 Codex 세션 UUID(또는 입력 문자열에서 추출)에 Auto-Resume 워커를 바인딩/해제":
        "绑定/解绑 Auto-Resume 工作器到目标 Codex 会话 UUID（或从输入字符串提取）",
    "워크플로우는 즉시 다음 노드로 진행되고, 워커는 백그라운드에서 토큰/레이트 한도 발생 시 codex --resume 으로 자동 재시도.":
        "工作流立即推进到下一节点；工作器在后台遇到令牌/速率限制时通过 codex --resume 自动重试。",
    "현재 적용 중인 모든 파라미터와 런타임 정보를 보여줍니다 (읽기 전용).":
        "显示当前所有活跃参数及运行时信息（只读）。",
    "여기에 적은 지침(기억·규칙·선호)은 모든 세션 시작 시 자동 로드됩니다. 직접 쓰거나, 위의":
    ...
    # Path fragments
    "/ 하위경로는":            "/ 子路径",
    "/AGENTS.md 로 기록됩니다.": "记录到 /AGENTS.md。",
}
