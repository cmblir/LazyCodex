...
NEW_EN: dict[str, str] = {
    ...
    "예: Researcher(Codex Sonnet) + Builder(Gemini Pro) + Reviewer(Ollama)":
        "e.g. Researcher (Codex Sonnet) + Builder (Gemini Pro) + Reviewer (Ollama)",
    ...
    "1~8명. 같은 역할이라도 모델이 다르면 별도 페르소나로 등록 가능 — Codex · Gemini · Ollama 자유 조합.":
        "1–8 personas. Same role with different models is fine — mix Codex · Gemini · Ollama freely.",
    "1~8명. 역할 + 모델 + 중점 영역 입력.":
        "1–8 personas. Enter role + model + focus area.",
    "Codex · Gemini · Ollama 자유 조합 — 서로 다른 모델을 섞을수록 다양한 시각을 얻습니다.":
        "Mix Codex · Gemini · Ollama freely — different models bring different perspectives.",
    "역할": "Role",
    ...
    "Slack 봇 토큰(xoxb-…) 을 한 번만 저장 — auth.test 로 즉시 검증. 토큰은 ~/.codex-dashboard-slack.json 에 chmod 600 으로 저장.":
        "Save the bot token (xoxb-…) once — verified via auth.test, stored at ~/.codex-dashboard-slack.json with chmod 600.",
    ...
    "가상 Codex 세션. subject(업무) · description(상세) · 모델 · cwd 를 설정하면 실행 시 codex -p 로 자동 실행되거나, 노드의 🖥️ 버튼으로 Terminal 새 창에서 대화형으로 열 수 있음.":
        "Virtual Codex session. Set subject (task) · description · model · cwd, then it runs via `codex -p` automatically, or click the 🖥️ button to open it interactively in a new Terminal window.",
    ...
    "LazyCodex · Crew Wizard 연결 테스트": "LazyCodex · Crew Wizard connection test",
    "메시지 전송": "message sent",
    ...
    "검사 대상: settings.json · AGENTS.md · settings.hooks · ~/.codex/agents · mcp.json":
        "Targets: settings.json · AGENTS.md · settings.hooks · ~/.codex/agents · mcp.json",
    "발견된 시크릿은 rotate 후 env var 로 옮기세요.":
        "Rotate any found secrets and move them to env vars.",
    "분석은 완전히 로컬에서 수행됩니다. Codex API 호출 없음. 제안 카드는 클릭 시 해당 탭으로 이동하여 수동 저장할 수 있습니다.":
        "Analysis is fully local — no Codex API calls. Click a suggestion card to jump to the relevant tab and save manually.",
    "최근 세션에서 반복되는 패턴을 자동 추출. AI 판단 없이 통계 기반.":
        "Extract recurring patterns from recent sessions — statistics only, no AI calls.",
    "최근 30일 세션이 없습니다. Codex CLI 를 한 번 이상 사용하세요.":
        "No sessions in the last 30 days. Run Codex CLI at least once.",
    ...
    "Codex CLI hooks 이벤트를 외부 HTTP endpoint 로 포워딩 — 화이트리스트 호스트만 허용, settings.json 자동 백업.":
        "Forward Codex CLI hook events to external HTTP endpoints — whitelist only, auto-backed-up settings.json.",
    "Codex CLI 세션 안에서 대시보드 기능을 호출할 수 있게 LazyCodex 를 stdio MCP 서버로 등록합니다":
        "Register LazyCodex as a stdio MCP server so dashboard features can be called from inside Codex CLI sessions.",
    "LazyCodex 자체를 MCP 서버로 (v2.32.0)": "LazyCodex as an MCP server (v2.32.0)",
    ...
    "~/.codex 전체(설정·훅·에이전트·MCP)를 정적 검사 — 시크릿 · 위험 훅 · 과도한 권한 · 신뢰 불가 MCP. AI 호출 없음, 100% 로컬.":
        "Statically scan all of ~/.codex (settings · hooks · agents · MCP) for secrets, risky hooks, over-broad permissions, and untrusted MCP servers. 100% local, no AI calls.",
...
NEW_ZH: dict[str, str] = {
    ...
    "예: Researcher(Codex Sonnet) + Builder(Gemini Pro) + Reviewer(Ollama)":
        "例如: Researcher (Codex Sonnet) + Builder (Gemini Pro) + Reviewer (Ollama)",
    "영문/숫자/공백/_-./ 만 허용 — Obsidian 폴더와 워크플로우 이름에 그대로 사용됩니다.":
    ...
    "작업 디렉터리 (선택)": "工作目录 (可选)",
    "1~8명. 같은 역할이라도 모델이 다르면 별도 페르소나로 등록 가능 — Codex · Gemini · Ollama 자유 조합.":
        "1–8 个角色。同角色不同模型可注册为独立角色 — Codex · Gemini · Ollama 自由组合。",
    "1~8명. 역할 + 모델 + 중점 영역 입력.":
        "1–8 个角色。填写角色 + 模型 + 关注领域。",
    "Codex · Gemini · Ollama 자유 조합 — 서로 다른 모델을 섞을수록 다양한 시각을 얻습니다.":
        "Codex · Gemini · Ollama 自由组合 — 不同模型带来不同视角。",
    "역할": "角色",
    "중점 영역 (선택)": "关注领域 (可选)",
    "시장조사": "市场调研",
    "페르소나 추가": "添加角色",
    ...
    "Slack 봇 토큰(xoxb-…) 을 한 번만 저장 — auth.test 로 즉시 검증. 토큰은 ~/.codex-dashboard-slack.json 에 chmod 600 으로 저장.":
        "保存 Bot Token (xoxb-…) 一次 — 通过 auth.test 即时验证。Token 存于 ~/.codex-dashboard-slack.json (chmod 600)。",
    ...
    "가상 Codex 세션. subject(업무) · description(상세) · 모델 · cwd 를 설정하면 실행 시 codex -p 로 자동 실행되거나, 노드의 🖥️ 버튼으로 Terminal 새 창에서 대화형으로 열 수 있음.":
        "虚拟 Codex 会话。设置 subject (任务) · description · 模型 · cwd 后，运行时通过 `codex -p` 自动执行；也可点击 🖥️ 按钮在新终端窗口交互运行。",
    "특정 역할(researcher · writer · reviewer 등)을 부여한 전문 세션. session 과 동일하지만 agentRole 필드로 페르소나를 명시해 전문성을 강화.":
    ...
    "LazyCodex · Crew Wizard 연결 테스트": "LazyCodex · Crew Wizard 连接测试",
    "메시지 전송": "消息已发送",
    ...
    "검사 대상: settings.json · AGENTS.md · settings.hooks · ~/.codex/agents · mcp.json":
        "检查目标: settings.json · AGENTS.md · settings.hooks · ~/.codex/agents · mcp.json",
    "발견된 시크릿은 rotate 후 env var 로 옮기세요.":
        "请轮换发现的密钥并迁移到环境变量。",
    "분석은 완전히 로컬에서 수행됩니다. Codex API 호출 없음. 제안 카드는 클릭 시 해당 탭으로 이동하여 수동 저장할 수 있습니다.":
        "分析完全在本地进行，无 Codex API 调用。点击建议卡片跳转到对应标签并手动保存。",
    "최근 세션에서 반복되는 패턴을 자동 추출. AI 판단 없이 통계 기반.":
        "从最近会话中自动提取重复模式 — 仅统计，无 AI 调用。",
    "최근 30일 세션이 없습니다. Codex CLI 를 한 번 이상 사용하세요.":
        "最近 30 天无会话。请至少运行一次 Codex CLI。",
    "아직 제안할 패턴이 충분히 누적되지 않았습니다.":
        "尚未累积足够的重复模式提供建议。",
    "스캔한 세션": "扫描的会话",
    "개 세션 중": " / 共",
    ...
    "Codex CLI hooks 이벤트를 외부 HTTP endpoint 로 포워딩 — 화이트리스트 호스트만 허용, settings.json 자동 백업.":
        "把 Codex CLI hook 事件转发到外部 HTTP endpoint — 仅白名单主机，settings.json 自动备份。",
    "Codex CLI 세션 안에서 대시보드 기능을 호출할 수 있게 LazyCodex 를 stdio MCP 서버로 등록합니다":
        "把 LazyCodex 注册为 stdio MCP 服务器，可在 Codex CLI 会话内调用仪表板功能。",
    "LazyCodex 자체를 MCP 서버로 (v2.32.0)": "LazyCodex 作为 MCP 服务器 (v2.32.0)",
    "노출되는 6 tool": "暴露的 6 个工具",
    "6 tool 노출: 탭 카탈로그 · 비용 요약 · 보안 스캔 · 러너 패턴 · RTK 상태 · 워크플로우 템플릿.":
        "暴露 6 个工具: 标签目录 · 成本汇总 · 安全扫描 · 学习器模式 · RTK 状态 · 工作流模板。",
    "설치 명령 (Terminal)": "安装命令 (终端)",
    "~/.codex 전체(설정·훅·에이전트·MCP)를 정적 검사 — 시크릿 · 위험 훅 · 과도한 권한 · 신뢰 불가 MCP. AI 호출 없음, 100% 로컬.":
        "对 ~/.codex 全部(设置·钩子·代理·MCP)进行静态检查 — 密钥 · 风险钩子 · 过度权限 · 不可信 MCP。100% 本地，无 AI 调用。",
