#!/usr/bin/env bash
# Build LazyCodex.app — a tiny macOS bundle that double-clicks into:
#   1) python3 server.py  (started in the background, logs to ~/Library/Logs/LazyCodex/server.log)
...
# is bundled — we depend on the system python3 (LazyCodex itself is stdlib-only).
#
# Usage:
#   tools/build_macos_app.sh                # builds dist/LazyCodex.app
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key>                <string>LazyCodex</string>
  <key>CFBundleDisplayName</key>         <string>LazyCodex</string>
  <key>CFBundleIdentifier</key>          <string>app.lazycodex.dashboard</string>
  <key>CFBundleVersion</key>             <string>${VERSION}</string>
  <key>CFBundleShortVersionString</key>  <string>${VERSION}</string>
  <key>CFBundleExecutable</key>          <string>LazyCodex</string>
  ...
  <key>NSHumanReadableCopyright</key>    <string>LazyCodex — local-first Codex CLI dashboard. MIT.</string>
</dict>
</plist>
PLIST

# ── 4. launcher script ──────────────────────────────────────────────────────
LAUNCHER="$APP/Contents/MacOS/LazyCodex"
cat > "$LAUNCHER" <<'SH'
#!/usr/bin/env bash
# LazyCodex .app launcher.
#   - finds the project directory (LAZYCODEX_HOME env > ~/Lazycodex > ~/lazycodex)
#   - starts python3 server.py in the background
#   - opens the dashboard in the default browser
#   - on receiving SIGTERM/SIGINT (Quit, killall LazyCodex, etc.) it terminates the server too
set -euo pipefail

LOG_DIR="$HOME/Library/Logs/LazyCodex"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/server.log"

# Resolve the LazyCodex project path. Tries:
#   1. $LAZYCODEX_HOME (env)
#   2. ~/Lazycodex
#   3. ~/lazycodex
#   4. /Applications/LazyCodex.app/../Lazycodex  (uncommon)
PROJ=""
for cand in "${LAZYCODEX_HOME:-}" "$HOME/Lazycodex" "$HOME/lazycodex"; do
  if [[ -n "$cand" && -f "$cand/server.py" ]]; then PROJ="$cand"; break; fi
done
if [[ -z "$PROJ" ]]; then
  /usr/bin/osascript -e 'display dialog "LazyCodex project not found.\n\nClone https://github.com/cmblir/LazyCodex into ~/Lazycodex or set the LAZYCODEX_HOME env var, then re-launch." buttons {"OK"} default button "OK" with icon caution with title "LazyCodex"' >/dev/null
  exit 1
fi
...
# Start the server.
echo "$(date '+%F %T')  starting LazyCodex server on $URL" >> "$LOG_FILE"
...
echo "✅ built $APP ($SIZE)"
echo "   double-click to launch (server logs: ~/Library/Logs/LazyCodex/server.log)"
