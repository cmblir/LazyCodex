...
Storage: ``~/.codex-dashboard-advisor-lab.json`` (history, ≤50).
"""
...
HISTORY_PATH = _env_path(
    "CODEX_DASHBOARD_ADVISOR_LAB",
    Path.home() / ".codex-dashboard-advisor-lab.json",
)
_MAX_HISTORY = 50

EXECUTORS = ["codex-haiku-4-5", "codex-sonnet-4-5", "codex-sonnet-4-6"]
ADVISORS  = ["codex-opus-4-6", "codex-opus-4-7"]
# (input, output) USD per 1M tokens
_PRICES = {
    "codex-haiku-4-5":  (1.0, 5.0),
    "codex-sonnet-4-5": (3.0, 15.0),
    "codex-sonnet-4-6": (3.0, 15.0),
    "codex-opus-4-6":   (15.0, 75.0),
    "codex-opus-4-7":   (15.0, 75.0),
}
...
def api_advisor_lab_run(body: dict) -> dict:
    ...
    executor = (body.get("executor") or "codex-haiku-4-5").strip()
    advisor  = (body.get("advisor")  or "codex-opus-4-7").strip()
    if executor not in EXECUTORS:
        executor = "codex-haiku-4-5"
    if advisor not in ADVISORS:
        advisor = "codex-opus-4-7"
