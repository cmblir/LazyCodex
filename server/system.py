...
def _running_sessions() -> list:
    ...
    for p in sorted(SESSIONS_DIR.glob("*.json")):
        ...
        out.append({
            ...
            "workspace": cwd,
            "project": Path(cwd).name if cwd else (data.get("kind") or "Codex CLI"),
            "kind": data.get("kind", ""),
...
def api_memory_list(query: dict) -> dict:
    """~/.codex/projects/*/memory/*.md 전부 나열."""
    out = []
...
def api_tasks_list() -> dict:
    """~/.codex/tasks/<sessionId>/*.json 태스크 전부 수집."""
    out = []
...
def api_task_save(body: dict) -> dict:
    ...
    sessionId 는 ~/.codex/tasks/<sessionId>/ 폴더. 기본 'dashboard-manual' 사용.
    """
...
OUTPUT_STYLES_DIR = CODEX_HOME / "output-styles"
PLANS_DIR = CODEX_HOME / "plans"
SHELL_SNAPSHOTS_DIR = CODEX_HOME / "shell-snapshots"
FILE_HISTORY_DIR = CODEX_HOME / "file-history"
BACKUPS_DIR = CODEX_HOME / "backups"
METRICS_COSTS_JSONL = CODEX_HOME / "metrics" / "costs.jsonl"
KEYBINDINGS_JSON = CODEX_HOME / "keybindings.json"
...
def api_metrics_summary() -> dict:
    """토큰 메트릭 — 세션 DB 가 진실 (costs.jsonl 은 Codex CLI 훅 버그로 0만 기록됨).
...
def api_backup_diff(query: dict) -> dict:
    ...
    두 경로 모두 ~/.codex/ 하위여야 함. 파일 크기 1MB 이하만 허용.
    """
    import difflib
    import os
    ...
    codex_home_real = os.path.realpath(str(CODEX_HOME))
    ...
    def _allowed(p: str) -> bool:
        try:
            ...
            return False
        return rp.startswith(codex_home_real + os.sep) or rp.startswith(home_real + os.sep)
...
TELEMETRY_DIR = CODEX_HOME / "telemetry"
HOMUNCULUS_JSON = CODEX_HOME / "homunculus" / "projects.json"
IMAGE_CACHE_DIR = CODEX_HOME / "image-cache"
...
def api_ide_status() -> dict:
    ...
    return {
        ...
        "note": "Codex CLI 는 현재 VS Code / JetBrains IDE 에 bridge 방식으로 연결됩니다. 연결된 IDE 는 세션 metadata 의 entrypoint/terminal 필드로 식별.",
    }
...
def api_homunculus_projects() -> dict:
    """~/.codex/homunculus/projects.json — Codex CLI 내부 프로젝트 추적."""
