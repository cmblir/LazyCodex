...
Lets the user post a prompt (+ optional screenshot) to Codex with the
...
Storage: ``~/.codex-dashboard-computer-use-lab.json`` (history, ≤50).
"""
...
HISTORY_PATH = _env_path(
    "CODEX_DASHBOARD_COMPUTER_USE_LAB",
    Path.home() / ".codex-dashboard-computer-use-lab.json",
)
_MAX_HISTORY = 50

MODELS = ["codex-sonnet-4-6", "codex-opus-4-7", "codex-sonnet-4-5", "codex-opus-4-6"]
...
def api_computer_use_run(body: dict) -> dict:
    ...
    model = (body.get("model") or "codex-sonnet-4-6").strip()
