"""Toolkits 설치 관리 — Everything Codex CLI(ECC) · Codex CLI Best Practice(CCB).

guideHub 탭의 툴킷 카드 3종 중 직접 설치/해제가 가능한 2개 관리.

스키마:
- ECC (Codex CLI plugin marketplace):
    `~/.codex/plugins/marketplaces/everything-codex-code/` 에 git clone.
    `~/.codex/plugins/known_marketplaces.json` 에 entry 등록하면 Codex CLI 가
    인식해 `/plugin install everything-codex-code@everything-codex-code` 로
    플러그인을 설치할 수 있게 된다.
    (실제 `/plugin install` 실행은 Codex CLI session 필요 → 사용자가 CC 에서 실행.)

- CCB (문서 레포):
    단순 `git clone` → 사용자가 지정한 경로(기본 `~/codex-code-best-practice`) 에 저장.
    설치/제거 = 디렉터리 존재 여부.

보안:
- 외부 호출은 `git clone` 고정 URL 만. subprocess 인자 배열 전달 (shell=False).
- 쓰기 경로는 ~/.codex/plugins/... · ~/… 아래로 제한.
"""
...
def _run_codex_plugin(args: list[str], timeout: int = 90) -> dict[str, Any]:
    """`codex plugin ...` subcommand — non-interactive.

    Used for full auto-install/uninstall/enable/disable of plugins without a CC session.
    """
    codex = _which("codex")
    if not codex:
        return {"ok": False, "error": "codex CLI not found", "error_key": "err_no_codex_cli"}
    try:
        proc = subprocess.run(
            [codex, "plugin", *args],
...
_ECC_ID = "everything-codex-code"
_ECC_REPO = "https://github.com/affaan-m/everything-codex-code.git"
_ECC_REPO_HTTPS = "https://github.com/affaan-m/everything-codex-code"

_CCB_ID = "codex-code-best-practice"
_CCB_REPO = "https://github.com/shanraisshan/codex-code-best-practice.git"
_CCB_REPO_HTTPS = "https://github.com/shanraisshan/codex-code-best-practice"
...
def api_toolkit_status(_q: dict | None = None) -> dict[str, Any]:
    ...
    # installed_plugins.json 에서 "@everything-codex-code" 들어간 plugin 검색
...
def api_toolkit_ecc_install(_body: dict | None = None) -> dict[str, Any]:
    ...
    1) `git clone https://github.com/affaan-m/everything-codex-code.git <PLUGINS_DIR>/marketplaces/everything-codex-code`
    2) `known_marketplaces.json` 에 entry 추가

    이후 Codex CLI 에서 `/plugin install everything-codex-code@everything-codex-code` 실행으로
    실제 플러그인 설치. 대시보드에서 자동 install 은 Codex CLI session 필요하여 지원 X.
    """
    ...
    known[_ECC_ID] = {
        "source": {"source": "github", "repo": "affaan-m/everything-codex-code"},
    ...
    return {
        ...
        "updated": updated,
        "path": str(target),
        "nextStep": f"Codex CLI 에서 `/plugin install {_ECC_ID}@{_ECC_ID}` 실행",
    }


def api_toolkit_ecc_install_plugin(body: dict | None = None) -> dict[str, Any]:
    """Run `codex plugin install everything-codex-code@everything-codex-code -s <scope>`.
    ...
    r = _run_codex_plugin(["install", target, "-s", scope])
    if not r.get("ok"):
...
def api_toolkit_ecc_uninstall_plugin(body: dict | None = None) -> dict[str, Any]:
    """Run `codex plugin uninstall everything-codex-code@everything-codex-code`."""
    body = body or {}
    scope = (body.get("scope") or "user").lower()
    if scope not in ("user", "project", "local"):
        return {"ok": False, "error": "invalid scope"}
    target = f"{_ECC_ID}@{_ECC_ID}"
    r = _run_codex_plugin(["uninstall", target, "-s", scope])
    if not r.get("ok"):
...
def api_toolkit_ecc_uninstall(_body: dict | None = None) -> dict[str, Any]:
    ...
    실제 설치된 플러그인(`installed_plugins.json`) 은 Codex CLI CLI 의 영역이라
    건드리지 않음. 사용자는 Codex CLI 에서 `/plugin uninstall` 로 별도 해제 필요.
    """
    target = _ecc_install_dir()
    removed_dir = False
    if target.exists():
    ...
    return {
        ...
        "note": "플러그인 자체는 Codex CLI 에서 `/plugin uninstall ...` 로 별도 제거",
    }
...
def api_toolkit_ccb_install(body: dict | None = None) -> dict[str, Any]:
    """CCB 레포 git clone — 기본 경로 `~/codex-code-best-practice/`.
