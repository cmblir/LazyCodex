"""`~/.codex/AGENTS.md` · `~/.codex/settings.json` 읽기/쓰기.

AGENTS.md 를 섹션 단위로 파싱하고, settings.json 의 permissions 필드를
...
from .config import AGENTS_MD, SETTINGS_JSON
...
# 권한 규칙 검증 (Codex CLI doctor 와 동일 룰)
...
def validate_permission_rule(rule: str) -> Optional[str]:
    ...
    if _PERM_INVALID_MIDPATTERN.search(rule):
        return (
            "Codex CLI 규칙: ':*' 는 패턴 맨 끝에만 올 수 있음. 중간에 쓰려면 '*' 사용. "
