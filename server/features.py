...
대시보드에서 Codex CLI 를 호출해 추천·평가를 수행하는 무거운 엔드포인트
들을 모아 둔 모듈. _template_* 는 AGENTS.md / 에이전트 .md / skill / hooks
초안 생성을 담당.
"""
...
from .config import (
    AGENTS_DIR, CODEX_HOME, CODEX_JSON, AGENTS_MD,
...
def api_optimization_score() -> dict:
    ...
    # === Agents: 사용자가 직접 만든 것 (~/.codex/agents) + 빌트인 ===
    ...
    score["agents"] = {
        ...
        "suggest": ("위임을 더 활용해보세요 — 정의된 에이전트가 30일간 적게 호출됨" if ag_q['quality'] < 0.5 and configured_agents > 0
                    else "~/.codex/agents/<name>.md 로 자신만의 서브에이전트 추가."),
        "target": "agents",
    }

    # === Skills: 사용자가 직접 만든 것 (~/.codex/skills) ===
    ...
    score["skills"] = {
        ...
        "note": f"사용자 설정 {user_skills_n}개 (~/.codex/skills) · 플러그인 제공 {plugin_skills_n}개는 'plugins' 축에서 평가",
        "formula": f"min(100, 사용자스킬×8) = min(100, {user_skills_n}×8) = min(100, {sk_raw})",
        "suggest": "~/.codex/skills/<id>/SKILL.md 로 자신만의 스킬 추가. 13개 = 만점.",
        "target": "skills",
    }
...
COST_LOG = CODEX_HOME / "cost-tracker.log"
BASH_LOG = CODEX_HOME / "bash-commands.log"
...
SCORE_FORMULA = {
    "engagement": {
        ...
        "howToIncrease": [
            ...
            "AGENTS.md에 '작업 시작 전 요구사항 확인' 지침 추가",
        ],
    },
    "productivity": {
    ...
    "delegation": {
        ...
        "howToIncrease": [
            ...
            "프로젝트 전용 에이전트(.codex/agents/*.md) 추가",
...
def _suggest_files_for_project(cwd: str, avg_breakdown: dict, repo: dict, settings: dict) -> list:
    ...
    # 1) AGENTS.md 없음
    if not (repo or {}).get("codexMd"):
        recs.append({
            "id": "create-codex-md",
            "title": "프로젝트 AGENTS.md 생성",
            "axis": "engagement",
            "impact": "+5~10",
            "reason": "AGENTS.md 는 세션 시작 시 자동 로드됩니다. 프로젝트 맥락(스택·규칙·도메인용어)을 여기 적으면 매 세션 설명 반복이 사라져 참여도·안정성 모두 상승.",
            "relpath": "AGENTS.md",
            "mode": "create",
            "content": _template_codex_md(cwd),
        })
    ...
    if not (repo or {}).get("agents"):
        recs.append({
            ...
            "axis": "delegation",
            "impact": "+10~15",
            "reason": "프로젝트 도메인에 특화된 에이전트를 `.codex/agents/<name>.md`에 두면 Agent 툴로 위임 가능. 위임(delegation) 축 점수가 직접 오릅니다.",
            "relpath": ".codex/agents/domain-expert.md",
            "mode": "create",
    ...
    if not (repo or {}).get("skills"):
        recs.append({
            ...
            "axis": "diversity",
            "impact": "+4~8",
            "reason": "반복 작업(배포/마이그레이션/PR 만들기 등)을 스킬로 정의하면 Codex가 자동 활용. 다양성 축을 올립니다.",
            "relpath": ".codex/skills/deploy/SKILL.md",
            "mode": "create",
            "content": _template_skill_md(),
        })

    # 4) .codex/settings.local.json 없음 or allow가 너무 빈약
    ...
    if allow_n < 6:
        recs.append({
            ...
            "relpath": ".codex/settings.local.json",
    ...
    if avg_breakdown.get("delegation", 0) < 3:
        recs.append({
            "id": "codex-md-delegation",
            "title": "위임 프롬프트를 AGENTS.md에 추가",
            "axis": "delegation",
            "impact": "+3~8",
            "reason": f"평균 위임 점수 {avg_breakdown.get('delegation',0)}. 'Explore/Plan 에이전트 우선 사용' 지침을 AGENTS.md 에 넣으면 반복 위임이 정착.",
            "relpath": "AGENTS.md",
            "mode": "append-codex-md",
            "content": (
                "\n## 🤝 에이전트 위임 우선순위\n"
...
def _template_codex_md(cwd: str) -> str:
    name = Path(cwd).name
    return f"""# {name}
...
def _template_hooks_patch(existing_settings: dict) -> str:
    ...
    ss.append({
        ...
        "hooks": [{
            "type": "command",
            "command": "echo '📋 이전 세션 요약은 $HOME/.codex/session-data/ 참조'",
            "timeout": 3000,
        }]
    })
...
def _format_existing_md_block(md: str) -> str:
    if not md:
        return ""
    return "## 현재 AGENTS.md 내용 (앞 1800자)\n" + md
...
def api_project_ai_recommend(body: dict) -> dict:
    """실제 codex CLI 를 subprocess 로 호출해 이 프로젝트 맞춤 추천을 JSON 으로 받음.
    cwd=프로젝트루트로 실행해서 Codex 가 실제 파일들을 읽을 수 있게 함.
    """
    ...
    if not auth.get("connected"):
        return {"error": "Codex 계정 연결이 필요합니다: " + (auth.get("reason") or "unknown")}

    # 컨텍스트 수집
    detail = api_project_score_detail({"cwd": [cwd]})
    if detail.get("error"):
        return detail
    repo = _scan_repo_local_codex(cwd)
    ...
    existing_md = (repo.get("codexMd") or "")[:1800]

    prompt = f"""당신은 Codex CLI 최적화 컨설턴트입니다. 아래 프로젝트 상태를 읽고, 점수를 올릴 수 있는 Codex 설정 파일 개선안을 JSON 으로만 답하세요.
...
## 현재 파일 상태
- AGENTS.md: {'있음 (' + str(len(repo.get('codexMd') or '')) + '자)' if repo.get('codexMd') else '없음'}
- .codex/agents: {len(repo.get('agents',[]))}개
- .codex/skills: {len(repo.get('skills',[]))}개
- .codex/commands: {len(repo.get('commands',[]))}개
- .codex/settings.local.json: {'있음' if repo.get('settingsLocal') else '없음'}
...
{{
  "recommendations": [
    {{
      ...
      "relpath": "AGENTS.md 또는 .codex/agents/<name>.md 또는 .codex/skills/<name>/SKILL.md 또는 .codex/settings.local.json",
      "mode": "create|edit|append-codex-md",
...
"""
    ...
    if not m:
        return {
            "error": "Codex 응답에서 recommendations JSON 을 찾지 못했습니다.",
...
_AI_EVAL_CACHE_FILE = _env_path(
    "CODEX_DASHBOARD_AI_EVAL_CACHE",
    Path.home() / ".codex-dashboard-ai-evaluation.json",
)


_LATEST_FEATURES_CACHE = _env_path(
    "CODEX_DASHBOARD_LATEST_FEATURES",
...
def api_design_exports(query: dict) -> dict:
    """Codex Design export 의심 파일을 스캔 (~/Downloads + 사용자 지정 경로).
    ...
    default_dirs = [
        ...
        str(Path.home() / "Documents" / "Codex Design"),
        str(Path.home() / "Desktop"),
    ]
    search_dirs = list(dict.fromkeys(default_dirs + extra_dirs))

    # Codex Design export 파일 패턴:
    # - .pdf, .pptx, .html, .canva 확장자
    # - 파일명에 'codex-design' 또는 'design-handoff' 포함 가능
    out = []
    seen = set()
    now = time.time()
        ...
        try:
            ...
            for item in p.iterdir():
                ...
                is_design_keyword = any(k in name_lower for k in
                    ("codex-design", "codex_design", "design-handoff", "handoff-bundle"))
    ...
    return {
        ...
        "files": out[:60],
        "hint": "Codex Design 공식 API 는 아직 미공개. codex.ai/design 에서 PDF/PPTX/HTML 로 export 한 파일을 기본 경로에서 스캔합니다.",
    }

def api_design_add_dir(body: dict) -> dict:
    """Codex Design export 를 저장하는 추가 디렉토리 등록."""
...
def api_ai_evaluation(body: dict) -> dict:
    """전체 셋업을 Codex 에게 평가받음. 비싸므로 force=true 시에만 새로 호출."""
    if not isinstance(body, dict):
        body = {}
    ...
    prompt = f"""당신은 Codex CLI 워크플로우 최적화 전문가입니다.
...
def api_feature_recommend(body: dict) -> dict:
    ...
    if kind == "hooks":
        ...
        system_intro = "사용자의 Codex CLI 를 위한 ~/.codex/settings.json 'hooks' 섹션을 만드세요."
        ...
        system_intro = "사용자의 ~/.codex/settings.json 'permissions' 를 최적화하세요."
        ...
        system_intro = "사용자를 위한 새 output style (~/.codex/output-styles/<name>.md) 초안을 작성하세요."
        output_shape = '{"id": "<kebab-name>", "raw": "---\\nname: ...\\n---\\n\\n본문"}'
        apply_hint = "~/.codex/output-styles/<id>.md 로 저장"
    elif kind == "statusline":
        current_obj = {"statusLine": s.get("statusLine"), "keybindings": KEYBINDINGS_JSON.exists()}
        system_intro = "~/.codex/settings.json 의 statusLine 설정을 추천하세요. 쉘 명령으로 current branch, model, cost 표시."
        ...
        system_intro = (
            "사용자의 작업 패턴에 맞는 Codex CLI 플러그인 활성화/추가를 추천하세요. "
        ...
        output_shape = (
            '{"recommendations": ['
            '{"mcpId":"<id from candidates>","why":"<workflow 에 어떻게 도움되는지>","cli":"<codex mcp add ...>","priority":1}'
            ']}'
        )
...
def api_global_codex_md_recommend(body: dict) -> dict:
    """사용자의 여러 프로젝트 활동을 종합해 글로벌 ~/.codex/AGENTS.md 초안 추천.
    body: {}
    """
    codex_bin = shutil.which("codex")
    if not codex_bin:
        return {"error": "codex CLI가 설치되어 있지 않습니다."}
    auth = api_auth_status()
    if not auth.get("connected"):
        return {"error": "Codex 계정 연결이 필요합니다."}
    ...
    # 기존 글로벌 AGENTS.md
    existing = _safe_read(AGENTS_MD) if AGENTS_MD.exists() else ""

    prompt = f"""당신은 Codex CLI 셋업 컨설턴트입니다. 사용자의 작업 패턴을 보고 ~/.codex/AGENTS.md (모든 세션에 자동 로드되는 글로벌 지침) 초안을 작성하세요.
...
## 출력 형식
JSON 만 반환: {{"content": "<AGENTS.md 전체 내용>"}}
"""
    lang = (body.get("lang") or "ko").lower()
    _lang_map = {"en": "\n\nIMPORTANT: Write the AGENTS.md content in English.", "zh": "\n\nIMPORTANT: Write the AGENTS.md content in Chinese (简体中文)."}
    if lang in _lang_map:
        prompt += _lang_map[lang]
    try:
        from .codex_exec import run_codex_exec
...
def api_project_file_put(body: dict) -> dict:
    ...
    - cwd 는 홈 하위여야 함
    - relpath 는 `.codex/…`, `AGENTS.md`, `~/.codex/settings.json` 중 하나만 허용
    """
    if not isinstance(body, dict):
        return {"ok": False, "error": "bad body"}
    cwd = body.get("cwd") or ""
    ...
    # 글로벌 settings 특례
    if relpath == "~/.codex/settings.json":
        try:
    ...
    allowed = (
        relpath == "AGENTS.md"
        or (rel_parts and rel_parts[0] == ".codex")
    )
    if not allowed:
        return {"ok": False, "error": "only AGENTS.md or .codex/** allowed"}
