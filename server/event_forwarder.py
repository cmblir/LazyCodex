"""Event Forwarder (v2.29.0) — Codex CLI hooks → 외부 HTTP POST.

사용자가 UI 에서 "이벤트 타입 + URL" 을 지정하면 `~/.codex/settings.json` 의
hooks 섹션에 `curl -s -X POST <url> --data-binary @-` 타입 엔트리를 추가한다.
이 훅은 Codex CLI 에서 해당 이벤트 발생 시 stdin 으로 이벤트 JSON 을
...
- URL 은 https-only + 호스트 화이트리스트 (SSRF 방지)
  ...
  - 사용자가 명시적으로 custom 호스트 추가 가능 (~/.codex-dashboard-config.json)
- settings.json 수정은 백업 자동 생성 (rtk_lab 패턴 재사용)
- forwarder 는 매 커맨드마다 고유 `lazycodex-forwarder` 마커를 command 에 포함 →
...
_MARKER = "__lazycodex_forwarder__"
