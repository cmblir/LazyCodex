...
Three read-only monitors plus two narrow mutating endpoints:
  ...
  - api_cli_sessions_list    live Codex CLI CLI sessions w/ RSS + idle
  ...
  - api_kill_idle_codex     bulk-SIGTERM idle CLI sessions
...
_CODEX_RE = re.compile(r"codex|codex-code|node.*codex", re.IGNORECASE)
...
def _top_processes(limit: int = 30) -> list[dict]:
    ...
    for line in out.splitlines():
        ...
        rows.append({
            ...
            "cpu_pct": cpu,
            "command": comm,
            "isCodexCode": bool(_CODEX_RE.search(comm)),
        })
...
# api_cli_sessions_list (full Codex sessions scan) — together ~360ms
...
def api_memory_snapshot(query: dict | None = None) -> dict:
    ...
    # idle Codex CLI session count — reuse cli_sessions_list result
...
def api_kill_idle_codex(body: dict) -> dict:
    """POST /api/memory/kill-idle-codex — SIGTERM all idle CLI sessions."""
    if not isinstance(body, dict):
        body = {}
    try:
