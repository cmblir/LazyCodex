...
- Spawn does **not** run anything — it just returns the resolved member list
  + the canonical ``codex /agents <name>`` invocation per agent so the UI can
...
AGENT_TEAMS_PATH = _env_path(
    "CODEX_DASHBOARD_AGENT_TEAMS",
    Path.home() / ".codex-dashboard-agent-teams.json",
)
...
def _agent_exists(member: dict) -> bool:
    ...
    if scope == "project" and cwd:
        return (Path(cwd) / ".codex" / "agents" / f"{name}.md").exists()
    if scope == "plugin":
        try:
            a = get_agent(name)
...
def api_agent_teams_spawn(body: dict) -> dict:
    ...
    Returns one entry per existing member with the canonical Codex CLI
    ...
    for m in team["agents"]:
        ...
        spawn_descs.append({
            ...
            "cwd":       cwd,
            "prompt":    prompt,
            "codexCmd": _build_codex_cmd(m.get("name"), prompt),
        })
    return {"ok": True, "teamId": tid, "spawn": spawn_descs, "skipped": skipped}


def _build_codex_cmd(agent_name: str, prompt: str) -> str:
    ...
    p = (prompt or "").replace('"', '\\"')
    if not p:
        return f'codex /agents {name}'
    return f'codex /agents {name} "{p}"'
