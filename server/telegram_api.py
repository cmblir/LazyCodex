...
`~/.codex-dashboard-telegram.json` (chmod 600), HTTPS only, hostname pinned to
...
TG_CONFIG_PATH = _env_path(
    "CODEX_DASHBOARD_TELEGRAM",
    Path.home() / ".codex-dashboard-telegram.json",
)
...
def _call(method: str, params: dict, token: Optional[str] = None,
    ...
    headers = {
        ...
        "User-Agent":   "LazyCodex-Telegram/2.55",
    }
    try:
...
def api_telegram_test(body: dict) -> dict:
    ...
    text = (body.get("text") or "LazyCodex · Telegram connection test").strip()
    try:
        me = get_me()
    except TelegramError as e:
