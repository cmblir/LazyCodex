"""Agent SDK 스캐폴드 — codex-agent-sdk 프로젝트 뼈대를 UI 로 생성.
...
def _python_main(template: str) -> str:
    base = '''"""codex-agent-sdk {T} 템플릿 — Python
...
def turn(user_text: str) -> str:
    ...
    resp = client.messages.create(model="codex-sonnet-4-6", max_tokens=512, messages=msgs)
...
def _ts_main(template: str) -> str:
    base = '''/**
 * codex-agent-sdk ''' + template + ''' 템플릿 — TypeScript
 *
 * 사전 준비:
 *   bun install     (처음 1회)
 *   export ANTHROPIC_API_KEY=sk-...
...
async function main() {
  ...
  const first = await client.messages.create({ model: "codex-sonnet-4-6", max_tokens: 1024, tools, messages: msgs });
  ...
  const second = await client.messages.create({ model: "codex-sonnet-4-6", max_tokens: 512, tools, messages: msgs });
...
async function turn(user: string) {
  ...
  const r = await client.messages.create({ model: "codex-sonnet-4-6", max_tokens: 512, messages: msgs });
...
`codex-agent-sdk` **{language}** / template: **{template}** (생성: codex-dashboard scaffold).
...
- [Codex API 문서](https://docs.anthropic.com/en/api/messages)
- [Codex Agent SDK](https://docs.anthropic.com/en/docs/codex-code/sdk/sdk-overview)
"""
...
name = "{name}"
version = "0.1.0"
description = "codex-agent-sdk {template} template"
...
def _open_terminal(cwd: Path, commands: list[str]) -> bool:
    ...
    script = f'''tell application "Terminal"
        activate
        do script "# codex-dashboard scaffold — 생성된 프로젝트\\n{joined}"
    end tell'''
    try:
