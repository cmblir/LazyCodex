"""RTK (Rust Token Killer) 통합 — Codex 토큰 60-90% 절감 (v2.24.0~, uninstall v2.25.0).

https://github.com/rtk-ai/rtk — Rust 로 작성된 CLI 프록시. `rtk git status`
처럼 감싸거나, `rtk init -g` 로 Codex CLI 에 Bash 훅을 설치하면 LLM 이
...
대시보드 통합 범위:
  ...
  - 훅 설치 여부 점검 (`~/.codex/settings.json` 내 rtk 참조 탐지)
...
def api_rtk_init(_body: dict | None = None) -> dict:
    """`rtk init -g --auto-patch` — Codex CLI 에 Bash 자동 재작성 훅 설치.
...
def api_rtk_uninstall_hook(_body: dict | None = None) -> dict:
    """Codex CLI settings.json 에서 rtk 관련 훅 entry 만 제거.
