...
def _cwd_to_slug(cwd: Path) -> str:
    """Codex CLI 의 프로젝트 슬러그 규칙: '/' → '-', 선두에 '-' 추가."""
...
SESSION_DATA_DIR = CODEX_HOME / "session-data"
TODOS_DIR = CODEX_HOME / "todos"
TASKS_DIR = CODEX_HOME / "tasks"
SCHEDULED_TASKS_DIR = CODEX_HOME / "scheduled-tasks"
...
MEMORY_DIR = _env_path(
    "CODEX_DASHBOARD_MEMORY_DIR",
    CODEX_HOME / "projects" / _cwd_to_slug(ROOT) / "memory",
)
DB_PATH = _env_path("CODEX_DASHBOARD_DB", Path.home() / ".codex-dashboard.db")
TRANSLATIONS_PATH = _env_path(
    "CODEX_DASHBOARD_TRANSLATIONS", Path.home() / ".codex-dashboard-translations.json"
)
DASHBOARD_CONFIG_PATH = _env_path(
    "CODEX_DASHBOARD_CONFIG", Path.home() / ".codex-dashboard-config.json"
)
WORKFLOWS_PATH = _env_path(
    "CODEX_DASHBOARD_WORKFLOWS", Path.home() / ".codex-dashboard-workflows.json"
)
