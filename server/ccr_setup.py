...
`@musistudio/codex-code-router` (`ccr`) package. The dashboard tab
guides the user through:

  1. Status check         (node / ccr / codex / config / port 3456)
  ...
  5. Shell alias snippet  (zcodex='ccr code')
...
CCR_CONFIG_DIR = Path.home() / ".codex-code-router"
...
_ALIAS_BEGIN = "# >>> zcodex (lazycodex) >>>"
_ALIAS_END = "# <<< zcodex (lazycodex) <<<"
_ALIAS_BODY = (
    f"{_ALIAS_BEGIN}\n"
    "alias zcodex='ccr code'\n"
    '# alternatively: eval "$(ccr activate)" && codex\n'
    f"{_ALIAS_END}\n"
)
...
def api_ccr_status(query: dict) -> dict:
    ...
    perf(v2.45.1): node/ccr/codex version probes + lsof port check are
...
def api_ccr_install_command(query: dict) -> dict:
    ...
    return {
        "ok": True,
        "command": "npm install -g @musistudio/codex-code-router",
        "note": (
...
def api_ccr_alias_snippet(query: dict) -> dict:
    ...
    return {
        ...
        "alternate_command": 'eval "$(ccr activate)" && codex',
