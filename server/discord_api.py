...
stored in ``~/.codex-dashboard-discord.json`` (chmod 600), HTTPS only via
...
DISCORD_CONFIG_PATH = _env_path(
    "CODEX_DASHBOARD_DISCORD",
    Path.home() / ".codex-dashboard-discord.json",
)
...
def _call(method: str, path: str, payload: Optional[dict] = None,
    ...
    headers = {
        ...
        "User-Agent":    "LazyCodex-Discord/2.56",
    }
...
def api_discord_test(body: dict) -> dict:
    ...
    text = (body.get("text") or "LazyCodex · Discord connection test").strip()
    try:
        me = get_current_user()
    except DiscordError as e:
