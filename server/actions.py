...
사이드 이펙트가 있는 엔드포인트들을 모아둔 모듈. 챗봇은 `codex -p` 를
...
def _build_chat_system_prompt() -> str:
    ...
    return f"""당신은 Codex Control Center 대시보드 v{version} 의 안내 도우미입니다.
...
def handle_chat_stream(handler: "Handler", body: dict) -> None:
    ...
    if not codex_bin:
        ...
        handler.send_header("Content-Type", "text/plain")
        handler.end_headers()
        handler.wfile.write(b"codex CLI not found")
        return
    ...
    # chat prompt for the same reason as QQ49 (codex-cli is text-only).
    if "data:image/" in full_prompt:
        try:
            ...
            if imgs:
                full_prompt = (cleaned + "\n\n[image attached — codex-cli has no vision]").strip()
        except Exception:
            pass
...
def _resolve_provider_cli(assignee: str) -> dict:
    ...
    prompt/options. Falls back to codex when the requested CLI isn't
    ...
    if pid == "gemini-cli":
        ...
        fallback_reason = "gemini CLI not installed — falling back to codex"
        pid = "codex-cli"
    elif pid == "ollama":
        ...
        fallback_reason = "ollama CLI not installed — falling back to codex"
        pid = "codex-cli"
    elif pid == "codex":
        b = _which_safe("codex")
        if b:
        ...
        fallback_reason = "codex CLI not installed — falling back to codex"
        pid = "codex-cli"

    # codex-cli (default + every fallback)
    codex_bin = _which_safe("codex") or "codex"
    return {"provider": "codex-cli", "bin": codex_bin, "args": [],
            "model": model if pid == "codex-cli" else "",
...
def api_session_spawn(body: dict) -> dict:
    ...
    body:
      cwd:                 "~/path"   (under $HOME)
      assignee:            "provider:model"  (optional — defaults to codex)
      prompt:              "first message"     (optional)
      systemPrompt:        "..."      (codex only)
      appendSystemPrompt:  "..."      (codex only)
      allowedTools:        "Bash,..." (codex only)
      disallowedTools:     "..."      (codex only)
      resumeSessionId:     "..."      (codex only)
    """
    if not isinstance(body, dict):
    ...
    # CLI interactively.
    cli_cmd = " ".join(parts)
    if prompt and provider == "codex-cli":
