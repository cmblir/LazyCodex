...
Scope:
- ``cwd=None`` → global writeable agent at ``~/.codex/agents/<name>.md``.
- ``cwd=<path>`` → project-scoped agent at ``<cwd>/.codex/agents/<name>.md``.
  (v2.40.0)

Storage:
- Agent body itself stays in its scope-native ``.md`` (Codex CLI compatible,
  no schema change).
- Meta + history live in ``~/.codex-dashboard-hyper-agents.json`` keyed by a
  composite ``<scope>:<id>``:
    * global: ``global:<name>``
...
HYPER_AGENTS_PATH = _env_path(
    "CODEX_DASHBOARD_HYPER_AGENTS",
    Path.home() / ".codex-dashboard-hyper-agents.json",
)
...
current Codex CLI sub-agent definition (frontmatter + system prompt body),
...
_AR_ADVISOR_SYSTEM_PROMPT = """You are an Auto-Resume advisor. Given a Codex CLI
