...
설계 원칙:
  1. 기존 `codex -p` 동작 완전 호환 (CodexCliProvider 가 기본)
...
class AIResponse:
    ...
    output: str = ""                # 생성된 텍스트
    error: str = ""                 # 에러 메시지
    provider: str = ""              # "codex-cli", "ollama", "openai-api", ...
...
class BaseProvider(ABC):
    ...
    # 서브클래스가 설정하는 메타데이터
    provider_id: str = ""           # "codex-cli", "ollama", "openai-api", ...
...
class OrchestratorProvider(BaseProvider):
    ...
    from the orchestrator config (``~/.codex-dashboard-orchestrator.json``)
...
class ProviderRegistry:
    ...
    def resolve_assignee(self, assignee: str) -> tuple[str, str]:
        ...
        # 프로바이더 없이 모델만 — Codex 별칭 확인
        if assignee.lower() in CodexCliProvider._ALIASES:
            return ("codex-cli", CodexCliProvider._ALIASES[assignee.lower()])
    ...
    def execute(
        ...
        *,
        ...
        UI surfaces "Codex rate-limited / OpenAI 401 / ..." instead of
        ...
        """
        attempts: list[str] = []   # ["codex-cli: rate limit ...", ...]
            ...
            if not is_hang:
                ...
                for alt_model in model_chain:
                    ...
                    if alt_resp.status == "ok":
                        ...
                        return alt_resp
                    attempts.append(f"codex-cli/{alt_model}: {(alt_resp.error or 'err')[:120]}")
