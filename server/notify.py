...
def _post_json(url: str, payload: dict) -> bool:
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json", "User-Agent": "LazyCodex/2.28"},
        )
...
def notify_workflow_completion(
    ...
    title = f"{emoji} LazyCodex · {wf_name}"
    body_lines = [
        f"run: {run_id}",
        f"status: {status}",
    ]
...
def send_email(cfg: dict, title: str, body: str) -> dict:
    ...
    try:
        ...
        msg["Subject"] = f"[LazyCodex] {title[:60]}"
        msg["From"] = sender
...
def send_telegram(cfg: dict, title: str, body: str) -> dict:
    ...
    try:
        ...
        req = urllib.request.Request(
            url, data=data, method="POST",
            headers={"Content-Type": "application/json", "User-Agent": "LazyCodex/2.49"},
        )
...
def api_notify_test(body: dict) -> dict:
    ...
    ok = (send_slack if target == "slack" else send_discord)(
        url, "LazyCodex · test message",
        "This is a test from LazyCodex workflow notification settings.",
    )
    return {"ok": ok}
