...
def _execute_node(node: dict, inputs: list[str], prev_session_ids: list[str] | None = None,
    ...
    # 항목이 있으면 그 body 를 systemPrompt 앞에 prepend 한다 (Legacy Team Pattern ultrathink 대응).
    try:
    ...
    # assignee 가 비어 있으면 기본 Codex provider:model 로 실행
...
BUILTIN_TEMPLATES: list[dict] = [
    ...
    # ── v2.25.0: Legacy Team Pattern 실행 모드 4종 (autopilot / ralph / ultrawork / deep-interview) ──
    {
        "id": "bt-autopilot", "name": "Autopilot", "icon": "🚀", "builtin": True,
        "description": "사용자 확인 없이 요구사항 → 실행 → 검증까지 단일 흐름으로 끝까지 돌리는 자율 파이프라인 (Autopilot 에 대응).",
        "category": "pattern",
        "nodes": [
    ...
    },
    {
        ...
        "description": "완료 기준 통과할 때까지 verify → fix 루프를 반복 (Ralph loop 에 대응). 최대 5회 반복, 피드백 자동 주입.",
        "category": "pattern",
        "nodes": [
    ...
    },
    {
        ...
        "description": "동일 작업을 5개 병렬 에이전트로 분할 실행 후 취합 (Ultrawork 에 대응). 속도 우선, 비용 5배.",
        "category": "pattern",
        "nodes": [
    ...
    },
    {
        ...
        "description": "모호한 요구사항을 Socratic 질문으로 명확화한 후 설계까지 (Deep Interview 에 대응).",
        "category": "pattern",
        "nodes": [
    ...
    # v2.27.0 — 팀 스프린트 5단계 파이프라인 (Plan → PRD → 3-병렬 Exec → Verify → Fix 루프)
    {
