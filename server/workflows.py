...
- 단일 JSON 파일(`~/.codex-dashboard-workflows.json`) 에 여러 워크플로우 저장
- 각 워크플로우 = 노드(세션·서브에이전트·취합·분기·결과) + 엣지(DAG)
- Run 엔진: DAG topological sort → 각 노드별 subprocess 로 `codex -p` 실행
...
_NODE_TYPES = {"start", "session", "subagent", "aggregate", "branch", "output",
               ...
               # v2.37.0 — bind a target Codex session to the Auto-Resume
...
# ~/.codex-dashboard-workflows.json on every transition. We persist to disk
...
#
# Runs migrated from the global ~/.codex-dashboard-workflows.json blob into
...
def _sanitize_node(raw: Any) -> Optional[dict]:
    ...
    if ntype in ("session", "subagent"):
        ...
        out["data"] = {
            ...
            # 세션 하네스 (persona · AGENTS.md 스타일 지시 · tools · resume)
        ...
        # v2.37.0 — bind a Codex CLI session UUID to the Auto-Resume
...
_FALLBACK_PROVIDERS = {"", "codex-api", "openai-api", "gemini-api", "ollama-api"}
...
def _sanitize_policy(raw: Any) -> dict:
    ...
    - `fallbackProvider` (v2.29.0): session 노드 실행 실패 시 재시도 프로바이더.
      `""` · `codex-api` · `openai-api` · `gemini-api` · `ollama-api` 중 하나.
    """
    if not isinstance(raw, dict):
...
def _run_status_snapshot(runId: str, *, full: bool = False) -> dict:
    ...
    # ~/.codex-dashboard-workflows.json on every SSE/poll tick. Falls back
...
def _execute_node(node: dict, inputs: list[str], prev_session_ids: list[str] | None = None,
    ...
    예: "codex:opus", "openai:gpt-4.1", "ollama:llama3.1", "gemini:2.5-pro", "codex:o4-mini"
    빈 값이거나 기존 형태(opus-4.7 등)면 Codex CLI 로 실행 (완전 호환).
    ...
    if not assignee and model_hint:
        if model_hint == "fast":
            chosen_model = "codex:haiku"
        elif model_hint == "deep":
            chosen_model = "codex:opus"
    ...
    # 기존 호환: 빈 값이거나 "opus-4.7" 같은 Codex alias 는 그대로 Codex CLI
    is_multi_provider = ":" in assignee and not assignee.startswith("codex-")

    # Codex CLI 전용 옵션 (다른 프로바이더에서는 extra 로 전달)
    extra = {
    ...
    try:
        ...
        if resp.status == "err" and fallback_provider and fallback_provider != (assignee or "codex-cli"):
            try:
...
def _execute_obsidian_log_node(data: dict, inputs: list[str], _elapsed) -> dict:
    ...
    project = (data.get("project") or "lazycodex").strip()
    ...
    for level in levels:
        ...
        for nid, res in level_results.items():
            ...
            if res.get("status") == "err":
                ...
                # at a 1024s timer on Codex/Gemini after GPT already failed.
                try:
                    _terminate_run_procs(runId)
                except Exception:
                    pass
...
BUILTIN_TEMPLATES: list[dict] = [
    {
        ...
        "description": "동일 프롬프트를 Codex, GPT, Gemini에 동시 전송하여 결과 비교",
        "category": "analysis",
        "nodes": [
            {"id": "n-start", "type": "start", "x": 80, "y": 200, "title": "시작", "data": {}},
            {"id": "n-codex", "type": "session", "x": 320, "y": 80, "title": "Codex", "data": {"subject": "분석 요청", "assignee": "codex:sonnet", "inputsMode": "concat"}},
        ...
        "edges": [
            {"id": "e1", "from": "n-start", "to": "n-codex", "fromPort": "out", "toPort": "in"},
            {"id": "e2", "from": "n-start", "to": "n-gpt", "fromPort": "out", "toPort": "in"},
            {"id": "e3", "from": "n-start", "to": "n-gemini", "fromPort": "out", "toPort": "in"},
            {"id": "e4", "from": "n-codex", "to": "n-merge", "fromPort": "out", "toPort": "in"},
    ...
    },
    {
        ...
        "nodes": [
            ...
            {"id": "n-gen", "type": "session", "x": 740, "y": 160, "title": "답변 생성", "data": {"subject": "검색 결과 기반 답변", "assignee": "codex:sonnet", "inputsMode": "concat"}},
    ...
    },
    {
        ...
        "nodes": [
            ...
            {"id": "n-sec", "type": "subagent", "x": 320, "y": 80, "title": "보안 리뷰", "data": {"subject": "보안 취약점 검사", "agentRole": "security-reviewer", "assignee": "codex:sonnet", "inputsMode": "concat"}},
            {"id": "n-review", "type": "subagent", "x": 320, "y": 280, "title": "코드 리뷰", "data": {"subject": "코드 품질 리뷰", "agentRole": "code-reviewer", "assignee": "codex:sonnet", "inputsMode": "concat"}},
    ...
    },
    {
        ...
        "nodes": [
            ...
            {"id": "n-analyze", "type": "session", "x": 680, "y": 160, "title": "AI 분석", "data": {"subject": "수집 데이터 분석", "assignee": "codex:sonnet"}},
    ...
    },
    {
        ...
        "nodes": [
            ...
            {"id": "n-work", "type": "session", "x": 480, "y": 160, "title": "작업 실행", "data": {"subject": "API 호출", "assignee": "codex:sonnet"}},
    ...
    },
    {
        ...
        "nodes": [
            ...
            {"id": "n-a", "type": "session", "x": 300, "y":  80, "title": "Agent A (Sonnet)",
             "data": {"subject": "작업의 1/5 담당 — 섹션 A 처리", "assignee": "codex:sonnet", "inputsMode": "concat"}},
            {"id": "n-b", "type": "session", "x": 300, "y": 180, "title": "Agent B (Sonnet)",
             "data": {"subject": "작업의 2/5 담당 — 섹션 B 처리", "assignee": "codex:sonnet", "inputsMode": "concat"}},
            {"id": "n-c", "type": "session", "x": 300, "y": 280, "title": "Agent C (Haiku)",
             "data": {"subject": "작업의 3/5 담당 — 섹션 C 처리", "assignee": "codex:haiku", "inputsMode": "concat"}},
            {"id": "n-d", "type": "session", "x": 300, "y": 380, "title": "Agent D (Haiku)",
             "data": {"subject": "작업의 4/5 담당 — 섹션 D 처리", "assignee": "codex:haiku", "inputsMode": "concat"}},
            {"id": "n-e", "type": "session", "x": 300, "y": 480, "title": "Agent E (Haiku)",
             "data": {"subject": "작업의 5/5 담당 — 섹션 E 처리", "assignee": "codex:haiku", "inputsMode": "concat"}},
        ...
        "nodes": [
            ...
            {"id": "n-plan",  "type": "session", "x": 280, "y": 240, "title": "🧭 기획자",
             "data": {"subject": "프로젝트 목표를 단계별 작업으로 쪼개고 페르소나에 분배",
                      ...
                      "assignee": "codex:opus", "agentRole": "planner",
            ...
            {"id": "n-obs",   "type": "obsidian_log", "x": 1200, "y": 240, "title": "📝 옵시디언 기록",
             "data": {"vaultPath": "~/ObsidianVault", "project": "lazycodex",
...
def api_workflow_runs_list(query: dict) -> dict:
    ...
    so callers like the terminal `lazycodex runs` verb and the LazyClaw
