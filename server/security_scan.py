...
SETTINGS = CODEX_HOME / "settings.json"
AGENTS_MD = CODEX_HOME / "AGENTS.md"
HOOKS_DIR = CODEX_HOME / "hooks"
AGENTS_DIR = CODEX_HOME / "agents"
MCP_JSON = CODEX_HOME / "mcp.json"
...
def _scan_settings(issues: list):
    if not SETTINGS.exists():
        _add(issues, "info", "integrity", "settings.json not found",
             "~/.codex/settings.json 이 없습니다.", str(SETTINGS))
        return
    try:
...
def _scan_codex_md(issues: list):
    ...
    try:
        ...
        return
    _scan_secrets_in_text(text, str(AGENTS_MD), issues)
    if len(text) > 50_000:
        _add(issues, "medium", "tokens",
             f"AGENTS.md is large ({len(text)} chars)",
