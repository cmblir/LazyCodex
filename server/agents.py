...
# ───────── 빌트인 에이전트 (Codex CLI 에 내장되어 파일 없음) ─────────
...
def list_agents() -> dict:
    ...
    for b in _BUILTIN_AGENTS:
        agents.append({**b, "scope": "builtin", "source": "Codex CLI", "path": "", "content": ""})
...
def _resolve_agent_path(agent_id: str) -> Optional[Path]:
    ...
    지원 형식:
      1. 전역: `agent-name` → ~/.codex/agents/<name>.md
      2. 3-part 플러그인 (대시보드 내부): `market:plugin:stem` → .../<market>/plugins/<plugin>/agents/<stem>.md
      3. 2-part 플러그인 (Codex CLI 호출 형식): `market:plugin` → 동일 market/plugin 의 첫 agent .md (또는 동명)
    """
    if ":" in agent_id:
...
def get_agent(agent_id: str) -> dict:
    ...
    for b in _BUILTIN_AGENTS:
        if b["id"] == agent_id:
            return {
                **b, "scope": "builtin", "raw": "",
                "content": "빌트인 에이전트는 Codex CLI에 내장되어 있어 파일로 편집할 수 없습니다.",
                "readOnly": True,
            }
...
def api_agent_create(body: dict) -> dict:
    """전역 ~/.codex/agents/<name>.md 생성. body: {name, description?, model?, tools?, content?}"""
