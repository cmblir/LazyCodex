...
~/.codex/projects/*/*.jsonl 을 SQLite 에 인덱싱하고, 세션/토큰/타임라인
...
from .config import PROJECTS_DIR, SCORE_MIN_TOOLS, SESSIONS_DIR, _UUID_RE, _cwd_to_slug
            ...
            for raw in f:
                ...
                if t == "session_meta":
                    ...
                    if isinstance(payload, dict):
                        ...
                        if not model and isinstance(payload.get("model_provider"), str):
                            model = payload.get("model_provider") or ""
                if t == "user":
                    user_cnt += 1
                    msg_count += 1
                    if not first_prompt:
                        ...
                        for b in content:
                            if isinstance(b, dict) and b.get("type") == "tool_use":
                                ...
                                if tool_name == "Agent" and subagent:
                                    ...
                                    edges.append((session_id, "codex", subagent, ts or 0))
                                input_summary = ""
...
def _iter_session_jsonls() -> list[tuple[Path, str]]:
    ...
    if PROJECTS_DIR.exists():
        for project_dir_path in sorted(PROJECTS_DIR.iterdir()):
            ...
            for jsonl in sorted(project_dir_path.glob("*.jsonl")):
                out.append((jsonl, project_dir_path.name))
    if SESSIONS_DIR.exists():
        for jsonl in sorted(SESSIONS_DIR.glob("**/*.jsonl")):
            out.append((jsonl, "codex-sessions"))
    return out


def index_all_sessions(force: bool = False) -> dict:
    """~/.codex/projects/*/*.jsonl 과 Codex CLI ~/.codex/sessions/**/*.jsonl 재인덱스."""
    _db_init()
    indexed = 0
    skipped = 0
    candidates = _iter_session_jsonls()
    if not candidates:
    ...
    for jsonl, project_dir in candidates:
        try:
...
def api_agent_graph(query: dict) -> dict:
    ...
    _ensure("codex", "core", "Codex (메인)")
    edge_list = []
    for r in edges:
        src, dst, n = r["src"], r["dst"], r["n"]
        _ensure(src, "core" if src == "codex" else "agent")
        _ensure(dst, "agent")
