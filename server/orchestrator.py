...
*planner* model — Codex by default, but never hardcoded), runs them in
...
Storage: ``~/.codex-dashboard-orchestrator.json`` — pure data, the orchestrator
itself contains *no* model names, channel ids, or routing rules in code.

::

    {
      "plannerAssignee":  "codex:gpt-5.5",
      "aggregatorAssignee": "codex:gpt-5.5",
      "defaultAssignees": ["codex:gpt-5.5", "openai:gpt-4.1", "ollama:llama3.1"],
      "maxParallel":      4,
      "channelDebounceMs": 2000,
      "bindings": [
        { "kind": "slack",    "channel": "C0XXXXX", "workflowId": "wf_..." },
        { "kind": "telegram", "chat":    "-100123",  "assignees": ["codex:gpt-5.5"] }
      ]
    }
...
ORCH_CONFIG_PATH = _env_path(
    "CODEX_DASHBOARD_ORCHESTRATOR",
    Path.home() / ".codex-dashboard-orchestrator.json",
)


# ───────── Defaults (env-overridable) ─────────

_DEFAULT_PLANNER     = os.environ.get("ORCH_PLANNER",     "codex:gpt-5.5")
_DEFAULT_AGGREGATOR  = os.environ.get("ORCH_AGGREGATOR",  "codex:gpt-5.5")
...
# with a private AGENTS.md so context from one channel doesn't leak into
# another. Disabled when the env var is unset.
_AGENT_WORKSPACE_ROOT = os.environ.get(
    "ORCH_AGENT_WORKSPACE_ROOT",
    str(Path.home() / ".codex-dashboard-agents"),
)
...
def _ensure_workspace(binding: Optional[dict]) -> dict:
    ...
    ``{"cwd": "...", "codexMdPath": "..."}`` or ``{}`` when isolation is off
    ...
    try:
        ...
        cmdpath = root / "AGENTS.md"
        if not cmdpath.exists():
            label = binding.get("label") or bid
            seed = (
                f"# Orchestrator workspace — {label}\n\n"
                f"This file is auto-generated by the LazyCodex orchestrator "
        ...
        return {"cwd": str(root), "codexMdPath": str(cmdpath)}
...
def _build_planner_prompt(user_text: str, available: list[str]) -> str:
    avail = ", ".join(available) or "codex:gpt-5.5"
...
def dispatch(text: str, *, kind: str = "http", channel: str = "",
    ...
    def _is_orch(a: str) -> bool:
        ...
        return head in ("orchestrator", "orch")
