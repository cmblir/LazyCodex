...
Given a project (anywhere on disk that has a ``AGENTS.md`` or git history),
...
- ``AGENTS.md`` at the project root or under ``.codex/`` — the canonical
...
returned as plain text. The draft includes:
  ...
  - Goal preamble (from AGENTS.md tagline if available, otherwise generic).
...
def _read_codex_md(root: Path) -> str:
    for cand in (root / "AGENTS.md",
...
def _extract_tagline(codex_md: str) -> str:
    m = _TAGLINE_RE.search(codex_md or "")
    if not m:
        return ""
...
def recommend(project_path: str) -> Optional[Recommendation]:
    ...
    if tagline:
        ...
        first = next((p.strip() for p in codex_md.split("\n\n")
    ...
    if not sections:
        sections.append("## Goal\nNo AGENTS.md, no TODOs, no git history. "
...
def _polish_system_prompt() -> str:
    ...
    Config file lives at ``$CODEX_DASHBOARD_RALPH_POLISH`` (defaults to
    ``~/.codex-dashboard-ralph-polish.md``); plain Markdown, no frontmatter.
    """
    ...
    try:
        ...
        cfg_path = _env_path("CODEX_DASHBOARD_RALPH_POLISH",
                             Path.home() / ".codex-dashboard-ralph-polish.md")
        if cfg_path.is_file():
