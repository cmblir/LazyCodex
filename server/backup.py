...
Single-archive backups to ~/.codex-dashboard-backups/<timestamp>.tar.gz
...
_HOME = Path.home()
_DB_FILE = _HOME / ".codex-dashboard.db"
_JSON_FILES: list[Path] = [
    _HOME / ".codex-dashboard-workflows.json",
    _HOME / ".codex-dashboard-auto-resume.json",
    _HOME / ".codex-dashboard-ai-providers.json",
    _HOME / ".codex-dashboard-slack.json",
    _HOME / ".codex-dashboard-prefs.json",
    _HOME / ".codex-dashboard-hyper-agents.json",
    _HOME / ".codex-code-router" / "config.json",
]
...
_ARCHIVE_NAME_MAP: dict[str, Path] = {
    ".codex-dashboard.db": _DB_FILE,
    ".codex-dashboard-workflows.json": _HOME / ".codex-dashboard-workflows.json",
    ".codex-dashboard-auto-resume.json": _HOME / ".codex-dashboard-auto-resume.json",
    ".codex-dashboard-ai-providers.json": _HOME / ".codex-dashboard-ai-providers.json",
    ".codex-dashboard-slack.json": _HOME / ".codex-dashboard-slack.json",
    ".codex-dashboard-prefs.json": _HOME / ".codex-dashboard-prefs.json",
    ".codex-dashboard-hyper-agents.json": _HOME / ".codex-dashboard-hyper-agents.json",
    "codex-code-router__config.json": _HOME / ".codex-code-router" / "config.json",
}


def _backup_root() -> Path:
    """Return the backups directory; create if missing."""
    p = _HOME / ".codex-dashboard-backups"
...
def api_backup_list(query: dict) -> dict:
    """List backups in ~/.codex-dashboard-backups, mtime desc."""
...
def api_backup_create(body: dict) -> dict:
    ...
    name = f"lazycodex-{ts}{suffix}.tar.gz"
    root = _backup_root()
    ...
    with tempfile.TemporaryDirectory(prefix="lcbk_") as tmpdir:
        ...
        # DB snapshot
        if _DB_FILE.exists():
            db_snap = staging / ".codex-dashboard.db"
