"""MCP 카탈로그 · 설치/삭제 · 커넥터 리스트 + `codex mcp list` 캐시.
...
MCP_CATALOG = [
    ...
    },
    {
        ...
        "cli": "codex mcp add github -e GITHUB_PERSONAL_ACCESS_TOKEN=... -- docker run -i --rm -e GITHUB_PERSONAL_ACCESS_TOKEN ghcr.io/github/github-mcp-server",
    },
    {
        "id": "filesystem",
        "name": "Filesystem",
        ...
        "cli": "codex mcp add filesystem npx -y @modelcontextprotocol/server-filesystem /Users/YOU/allowed-path",
    },
    {
        "id": "memory",
        ...
        "cli": "codex mcp add memory npx -y @modelcontextprotocol/server-memory",
    },
    {
        "id": "playwright",
        "name": "Playwright",
        ...
        "cli": "codex mcp add playwright npx -y @microsoft/mcp-server-playwright",
    },
    {
        "id": "puppeteer",
        "name": "Puppeteer",
        ...
        "cli": "codex mcp add puppeteer npx -y @modelcontextprotocol/server-puppeteer",
    },
    {
        "id": "brave-search",
        "name": "Brave Search",
        ...
        "install": {"type": "stdio", "command": "npx",
                    ...
                    "env": {"BRAVE_API_KEY": ""}},
        "cli": "codex mcp add brave-search -e BRAVE_API_KEY=... -- npx -y @modelcontextprotocol/server-brave-search",
    },
    {
        "id": "fetch",
        "name": "Fetch",
        ...
        "cli": "codex mcp add fetch uvx mcp-server-fetch",
    },
    {
        "id": "sqlite",
        "name": "SQLite",
        ...
        "cli": "codex mcp add sqlite uvx mcp-server-sqlite --db-path /absolute/path.db",
    },
    {
        "id": "postgres",
        "name": "PostgreSQL",
        ...
        "cli": "codex mcp add postgres npx -y @modelcontextprotocol/server-postgres postgresql://user:pass@host/db",
    },
    {
        "id": "slack",
        "name": "Slack",
        ...
        "cli": "codex mcp add slack -e SLACK_BOT_TOKEN=... -e SLACK_TEAM_ID=... -- npx -y @modelcontextprotocol/server-slack",
    },
    {
        "id": "google-drive",
        "name": "Google Drive",
        ...
        "cli": "codex mcp add gdrive npx -y @modelcontextprotocol/server-gdrive",
    },
    {
        ...
        "cli": "codex mcp add seq-think npx -y @modelcontextprotocol/server-sequential-thinking",
    },
    {
        "id": "exa",
        "name": "Exa Search",
        ...
        "install": {"type": "stdio", "command": "npx", "args": ["-y", "exa-mcp-server"],
                    "env": {"EXA_API_KEY": ""}},
        "cli": "codex mcp add exa -e EXA_API_KEY=... -- npx -y exa-mcp-server",
    },
    {
        "id": "linear",
        "name": "Linear",
        ...
        "install": {"type": "stdio", "command": "npx", "args": ["-y", "linear-mcp"],
                    "env": {"LINEAR_API_KEY": ""}},
        "cli": "codex mcp add linear -e LINEAR_API_KEY=... -- npx -y linear-mcp",
    },
    {
        "id": "notion",
        "name": "Notion",
        ...
        "cli": "codex mcp add notion -e INTERNAL_INTEGRATION_TOKEN=... -- npx -y @makenotion/notion-mcp-server",
    },
]
...
_MCP_LIST_CACHE_FILE = _env_path(
    "CODEX_DASHBOARD_MCP_CACHE",
    Path.home() / ".codex-dashboard-mcp-cache.json",
)
...
def _refresh_mcp_list_blocking() -> tuple[dict, dict]:
    """실제 `codex mcp list` 호출 (단일 in-flight 보장)."""
...
def _codex_mcp_list_cached() -> tuple[dict, dict]:
...
def list_connectors() -> dict:
    ...
    if CODEX_DESKTOP_CONFIG.exists():
        try:
            d_data = json.loads(_safe_read(CODEX_DESKTOP_CONFIG))
        except Exception:
            d_data = {}
        ...
        if isinstance(d_mcp, dict):
            for name, cfg in d_mcp.items():
                ...
                desktop_list.append({
                    ...
                    "configPath": str(CODEX_DESKTOP_CONFIG),
                })

    cli_status, cli_url = _codex_mcp_list_cached()

    # needs-auth 캐시 (플랫폼 MCP fallback)
    auth_cache_path = CODEX_HOME / "mcp-needs-auth-cache.json"
    needs_auth: dict = {}
    ...
    # needs-auth 캐시에서만 확인되는 platform 항목 보강 (codex CLI 미설치 fallback)
