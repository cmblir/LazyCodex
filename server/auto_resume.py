...
the user specified:
    ...
    7. Observable state file         -> ~/.codex-dashboard-auto-resume.json
...
Storage
-------
~/.codex-dashboard-auto-resume.json
    {
      "<session_id>": {
        "sessionId":        str,
        "enabled":          bool,
...
from .config import _env_path, CODEX_HOME
...
AUTO_RESUME_PATH = _env_path(
    "CODEX_DASHBOARD_AUTO_RESUME",
    Path.home() / ".codex-dashboard-auto-resume.json",
)
...
_PAT_AUTH_EXPIRED = re.compile(
    r"\b(unauthori[sz]ed|invalid api key|please run\s+/login|please run\s+codex\s+login|"
...
def _snapshot_dir(cwd: str) -> Path:
    return Path(cwd) / ".codex" / "auto-resume"
...
def _resolve_jsonl(session_id: str, cwd: str = "") -> Optional[Path]:
    if not session_id:
        return None
    projects_dir = CODEX_HOME / "projects"
    if projects_dir.exists() and cwd:
    ...
    try:
        with jsonl.open("r", encoding="utf-8", errors="replace") as fh:
            for _ in range(50):
                ...
                c_val = msg.get("cwd") or payload.get("cwd")
...
def _send_notify(entry: dict, kind: str, summary: str) -> None:
    ...
    try:
        ...
        cwd = entry.get("cwd") or "?"
        title = f"{emoji} LazyCodex Auto-Resume · {kind} · {sid[:8]}"
...
def _live_cli_sessions() -> dict:
    """Return {sessionId: liveRecord} for currently-running Codex CLI CLI sessions.
...
def api_auto_resume_set(body: dict) -> dict:
    ...
    if jsonl is None:
        return {"ok": False, "error": "session jsonl not found under ~/.codex/projects"}
    if not cwd:
    ...
    if _codex_bin() is None:
        return {"ok": False, "error": "`codex` binary not on PATH"}
...
def api_auto_resume_inject_live(body: dict) -> dict:
    ...
    # stale if the user reopened codex.
    live_map = _live_cli_sessions()
...
def _spawn_resume(entry: dict) -> tuple[int, str, str]:
    """Run `codex` once and return (exit_code, stderr_tail, stdout_tail)."""
    bin_path = _codex_bin()
    if bin_path is None:
        return (127, "`codex` binary not on PATH", "")
...
def _process_one(session_id: str) -> None:
    ...
    with _LOCK:
        ...
        if reason == "clean":
            ...
            e["state"] = STATE_FAILED
            e["enabled"] = False
            e["stopReason"] = "auth expired — run `codex /login` and re-enable"
            notify_kind = "failed"
            notify_summary = "auth_expired — run `codex /login` and re-enable"
        elif stalled:
