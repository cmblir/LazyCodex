"""LazyCodex MCP Server (v2.32.0) — stdio JSON-RPC 2.0.

Codex CLI 세션에서 `/lazycodex/<tool>` 로 대시보드 기능을 호출할 수 있게
LazyCodex 를 Model Context Protocol 서버로 노출한다.

전송: stdio (newline-delimited JSON-RPC)
프로토콜: MCP 2024-11-05

등록 방법:
    codex mcp add lazycodex -- python3 /Users/o/codex-dashboard/scripts/lazycodex_mcp.py

노출 tools:
- lazycodex_tabs               : 전체 탭 카탈로그 (id/label/desc)
- lazycodex_cost_summary       : 비용 타임라인 요약
- lazycodex_security_scan      : ~/.codex 정적 보안 검사
- lazycodex_learner_patterns   : 세션 반복 패턴
- lazycodex_rtk_status         : RTK 설치/훅 상태
- lazycodex_workflow_templates : 빌트인 워크플로우 템플릿 목록
"""
...
def _tool_workflow_templates(_args: dict) -> str:
    ...
    out = ["# LazyCodex Built-in Workflow Templates\n"]
...
TOOLS = [
    {
        "name": "lazycodex_tabs",
        "description": "List all LazyCodex dashboard tabs grouped by category (Learn/Main/Build/Playground/Config/Observe) with descriptions.",
        "inputSchema": {"type": "object", "properties": {}, "required": []},
        "_handler": _tool_tabs,
    },
    {
        "name": "lazycodex_cost_summary",
        "description": "Summarize LazyCodex unified cost timeline — total USD, top sources (promptCache/thinkingLab/workflows/...), top models.",
        "inputSchema": {"type": "object", "properties": {}, "required": []},
        "_handler": _tool_cost_summary,
    },
    {
        "name": "lazycodex_security_scan",
        "description": "Run a static security scan over ~/.codex (settings, AGENTS.md, hooks, agents, MCP) for secret leaks, risky hooks, over-privileged permissions. 100% local, no AI.",
        "inputSchema": {"type": "object", "properties": {}, "required": []},
        "_handler": _tool_security_scan,
    },
    {
        "name": "lazycodex_learner_patterns",
        "description": "Extract repeated tool sequences and prompts from recent Codex CLI sessions — pure statistics (no AI).",
        "inputSchema": {"type": "object", "properties": {}, "required": []},
        "_handler": _tool_learner_patterns,
    },
    {
        "name": "lazycodex_rtk_status",
        "description": "Report RTK (Rust Token Killer) install + hook status from the dashboard.",
        "inputSchema": {"type": "object", "properties": {}, "required": []},
        "_handler": _tool_rtk_status,
    },
    {
        "name": "lazycodex_workflow_templates",
        "description": "List LazyCodex built-in workflow templates (autopilot, ralph, ultrawork, deep-interview, team-sprint, etc.) with node/edge counts.",
        "inputSchema": {"type": "object", "properties": {}, "required": []},
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "lazycodex", "version": get_version()},
            },
        }
...
def api_mcp_server_info(_q: dict | None = None) -> dict:
    """대시보드 UI 용 — LazyCodex MCP 서버 진입점 스크립트 경로 + 노출 도구 목록."""
    from pathlib import Path
    script = Path(__file__).resolve().parent.parent / "scripts" / "lazycodex_mcp.py"
    return {
        "ok": True,
        "path": str(script),
