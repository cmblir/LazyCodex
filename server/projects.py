...
- list_projects : ~/.codex/projects/<slug>/ 스캔
- api_project_detail : 프로젝트별 AGENTS.md/settings.json/에이전트/세션 집계
- api_project_score_detail / _tool_breakdown : 분석 뷰
- AGENT_ROLE_CATALOG : 16개 역할 프리셋 (프론트엔드가 프리셋 사용)
- api_project_agent_add/delete/save : <cwd>/.codex/agents/*.md 편집
...
from .config import (
    AGENTS_DIR, CODEX_JSON, PROJECTS_DIR, SCORE_MIN_TOOLS,
)
...
def list_projects() -> dict:
    ...
    for p in sorted(PROJECTS_DIR.iterdir()):
        ...
        if cwd:
            try:
                has_codex_md = (Path(cwd) / "AGENTS.md").exists()
            except Exception:
                has_codex_md = False
        ...
        projects.append({
            ...
            "metaDir": str(p),           # ~/.codex/projects/<slug>
            "cwdResolved": bool(cwd),
            "hasCodexMd": has_codex_md,
            "sessionCount": len(session_files),
        })
    return {"projects": projects}


def _scan_repo_local_codex(cwd: str) -> dict:
    """<cwd>/.codex 디렉토리에서 에이전트/커맨드/훅/스킬/settings 전부 스캔."""
    out = {
        "exists": False,
        "codexMd": None,
        "codexMdPath": "",
    ...
    # AGENTS.md (repo root)
    codex_md = base / "AGENTS.md"
    if codex_md.exists():
        out["codexMd"] = _safe_read(codex_md, 20000)
        out["codexMdPath"] = str(codex_md)
...
def api_project_detail(query: dict) -> dict:
    """프로젝트 cwd 기준 스냅샷: 저장소-로컬 .codex + ~/.codex.json 프로젝트 엔트리 + 세션 목록."""
    ...
    # 저장소-로컬 설정
    repo = _scan_repo_local_codex(abs_path)

    # ~/.codex.json 내 per-project 엔트리
    project_entry = {}
...
def api_project_score_detail(query: dict) -> dict:
    ...
    # 프로젝트-로컬 상태
    repo = _scan_repo_local_codex(cwd_resolved) if cwd_resolved else {}
    ...
    return {
        ...
        "repoHas": {
            "codexMd": bool(repo.get("codexMd")) if repo else False,
...
def api_project_agent_add(body: dict) -> dict:
    ...
    target = Path(cwd) / ".codex" / "agents" / f"{name}.md"
...
def api_subagent_set_model(body: dict) -> dict:
    ...
    if scope == "project":
        ...
        target = Path(cwd) / ".codex" / "agents" / f"{agent_id}.md"
    elif scope == "global":
        if not re.match(r"^[a-zA-Z0-9_-]+$", agent_id):
            return {"ok": False, "error": "invalid agent name"}
...
# Global ↔ Project parity for AGENTS.md / settings.json / settings.local.json /
# skills / commands. Every path resolves under $HOME or we refuse the write.
# Frontend sends `cwd` (absolute path); backend derives `<cwd>/AGENTS.md` etc.
...
def api_project_codex_md_get(query: dict) -> dict:
    """GET /api/project/codex-md?cwd=... — read project AGENTS.md."""
...
def api_project_codex_md_put(body: dict) -> dict:
    """PUT /api/project/codex-md  body: {cwd, raw}."""
    if not isinstance(body, dict):
        return {"ok": False, "error": "bad body"}
...
def _project_settings_get(cwd: Path, filename: str) -> dict:
    """Shared: read `<cwd>/.codex/<filename>` as JSON or {} if missing."""
    p = cwd / ".codex" / filename
    if not p.exists():
...
def _project_settings_put(cwd: Path, filename: str, body: dict) -> dict:
    """Shared: write `<cwd>/.codex/<filename>` from `body.raw` (string) or
    `body.data` (object). Sanitises permissions like `put_settings` does."""
    from .codex_md import sanitize_permissions, validate_permissions
    raw = body.get("raw")
    data = body.get("data")
...
# `_scan_repo_local_codex` (above) already lists them read-only. These add
...
def api_project_skills_list(query: dict) -> dict:
    ...
    if not cwd:
        return {"ok": False, "error": "invalid or out-of-home cwd"}
    repo = _scan_repo_local_codex(str(cwd))
...
def api_project_skill_get(query: dict) -> dict:
    ...
    p = cwd / ".codex" / "skills" / sid / "SKILL.md"
    if not p.exists():
...
def api_project_command_get(query: dict) -> dict:
    ...
    base_dir = (cwd / ".codex" / "commands").resolve()
    p = (cwd / ".codex" / "commands" / (rel + ".md")).resolve()
