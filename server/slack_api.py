...
- Token loaded from `~/.codex-dashboard-slack.json` (chmod 600).
...
SLACK_CONFIG_PATH = _env_path(
    "CODEX_DASHBOARD_SLACK",
    Path.home() / ".codex-dashboard-slack.json",
)
...
def api_slack_test(body: dict) -> dict:
    ...
    msg = (body.get("text") or "LazyCodex · Slack connection test").strip()
    try:
        auth = auth_test()
    except SlackError as e:
