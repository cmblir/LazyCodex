# OpenClaw / NanoClaw / Ralph 분석 + LazyCodex 통합 기획

**작성일**: 2026-05-01
**근거**: [§1.2 정직성](../../../AGENTS.md) 준수 — 추측 대신 실제 1차 자료(GitHub README, 공식 plugin docs, 작성자 블로그) 기준.
...
- **per-agent 워크스페이스**: 자체 AGENTS.md / memory / 컨테이너.
...
- Anthropic 공식 Codex Agent SDK 기반.

### Ralph (Ralph Wiggum loop)
- 작성자: Geoffrey Huntley. Anthropic 공식 plugin: `codex-code/plugins/ralph-wiggum`.
- 핵심: **Stop hook**이 Codex의 종료를 가로채서 같은 PROMPT.md를 다시 주입. 외부 bash loop 불필요 — 한 세션 내부에서 반복.
- 상태는 **파일 + git history**로만 유지 (프롬프트는 불변). Codex가 매 iteration에서 자기 이전 변경사항을 읽고 개선.
...
## 2. LazyCodex 현황 (cycle 1-3 완료분)

| 영역 | 보유 | 부족 |
|---|---|---|
| 멀티 모델 디스패치 | ✓ `execute_with_assignee` (codex/openai/gemini/ollama/codex) | per-binding failover (전역 fallback chain만) |
...
- 입력 자동수집: 해당 프로젝트의 `AGENTS.md`, 최근 git log 30개, 미해결 TODO/FIXME 목록(grep), 최근 5개 세션의 미완 작업, 실패한 마지막 워크플로우 run.
...
- 전역 fallback chain만 있는 걸 `binding.fallbackChain: ["codex:opus", "codex:sonnet", "openai:gpt-4.1"]`로 확장.
...
#### **T2-3. Agent 격리 워크스페이스 (ephemeral AGENTS.md)**
- NanoClaw의 per-agent isolated AGENTS.md를 차용.
- `~/.codex-dashboard-agents/<binding-id>/AGENTS.md` + `memory/` — 한 채널의 컨텍스트가 다른 채널을 오염시키지 않음.
