...
- `server.py` (root) — entry. Resolves port conflicts (kills the existing occupant of `$PORT`, default 19500), initialises SQLite (`~/.codex-dashboard.db`), background-indexes Codex CLI sessions, warms MCP cache, starts the workflow scheduler, auto-starts `ollama serve`, then `ThreadingHTTPServer(Handler)`.
...
- `server/ai_providers.py` + `server/ai_keys.py` — provider registry. `execute_with_assignee("provider:model", prompt, ...)` is the unified entry — `assignee` like `codex:opus`, `openai:gpt-4.1`, `ollama:llama3.1`. Custom CLI providers register here.
...
|---|---|---|
| `~/.codex-dashboard-workflows.json` | Workflows + runs + custom templates + history | `CODEX_DASHBOARD_WORKFLOWS` |
| `~/.codex-dashboard-ai-providers.json` | API keys, custom CLIs, fallback chain | `CODEX_DASHBOARD_AI_PROVIDERS` |
| `~/.codex-dashboard-slack.json` | Bot token (chmod 600) + default channel | `CODEX_DASHBOARD_SLACK` |
| `~/.codex-dashboard.db` | SQLite — session index, costs, telemetry | `CODEX_DASHBOARD_DB` |
| `~/.codex/` | Codex CLI's own state — read-only from this app | `CODEX_HOME` |
...
- **No main pushes.** `~/.codex/AGENTS.md` §4.6 forbids `git push origin main`. Work on `feat/*` or `fix/*` branches and push there.
