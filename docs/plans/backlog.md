...
- **B7. Event Forwarder 탭** — `~/.codex/settings.json` hooks 를 UI 에서 직접 조립 (PostToolUse · OnSessionStart 등) → 외부 HTTP POST. 기존 hooks 탭 확장으로도 가능.
- **B8. Learner 탭** — 최근 세션 JSONL 분석 → 반복 패턴 추출 → skill/prompt 후보 제안. AI 판단 정확도 검증 필요.

## LazyCodex 자체 개선

- **MCP 서버 모드** — `lazycodex` 를 MCP 서버로 등록 → Codex CLI 세션에서 `/lazycodex/<tab>` 으로 대시보드 기능 호출. OMC/OMX 의 "세션 내 통합" 경쟁력 확보.
