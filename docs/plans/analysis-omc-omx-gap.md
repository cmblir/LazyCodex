# OMC / OMX vs LazyCodex — Gap Analysis

생성: 2026-04-23
레퍼런스: 
- `Yeachan-Heo/oh-my-codexcode` (OMC, TS, 30.8k ★) — Codex CLI 용 팀 오케스트레이션
- `Yeachan-Heo/oh-my-codex` (OMX, TS, 25.2k ★) — Codex 용 동일 계열
- 현재 상태: LazyCodex v2.24.1 (53 탭, 192 API 라우트, 3,291 i18n)

## 핵심 차별화 축

| 축 | OMC | OMX | LazyCodex (v2.24.1) |
|---|---|---|---|
...
## LazyCodex 가 이미 우위인 영역

- **시각적 설정 관리 전체** — agents / skills / hooks / plugins / MCP / commands / statusline / permissions / settings / codexmd / env / model config. OMC·OMX 는 CLI 만 제공.
- **n8n 스타일 DAG 워크플로우 엔진** — 16 노드 타입 × 5 빌트인 템플릿 × 조건부 11종 × webhook · cron · repeat.
- **Codex API 플레이그라운드 11 탭** — prompt cache / thinking / tool-use / batch / files / vision / model bench / server tools / citations / agent SDK 스캐폴드 / embedding. OMC·OMX 에는 없음.
...
LazyCodex 는 **대시보드/플랫폼 레이어**, OMC/OMX 는 **세션 내 워크플로우 레이어**. 직접 대체 관계가 아니라 서로 다른 위치.

---

## OMC/OMX 에 있고 LazyCodex 에 없는 것 — 흡수 후보 (우선순위 높 → 낮)

### 🔴 HIGH — 핵심 차별화 가치 큼

1. **멀티 에이전트 팀 오케스트레이션 (OMC `omc team 3:executor` / OMX `$team`)**  
   현재 워크플로우는 단일 session/subagent 노드 DAG. 팀 개념 없음.  
   → LazyCodex 에 **Team 노드** 추가: `Plan → PRD → Exec(N병렬) → Verify → Fix` 파이프라인을 프리셋 템플릿으로. 기존 DAG 위에 구축 가능.

2. **실행 모드 프리셋 — autopilot / ralph / ultrawork / deep-interview (OMC)**  
   한 번의 지시로 자동 verify-fix 루프를 도는 모드. LazyCodex 의 Repeat + feedback node 는 있지만 "persistent until completion" 정책이 명시적이지 않음.  
...
4. **Session 분석 — JSONL replay + JSON 요약 (OMC `.omc/sessions/`)**  
   현재 sessions 탭은 Codex CLI JSONL 을 읽지만 per-run diff/replay UI 는 워크플로우 run 에만 한정.  
...
5. **알림 라우팅 (OMC `omc config-stop-callback` → Telegram/Discord/Slack)**  
   워크플로우 완료/실패를 외부 채널로. LazyCodex 는 브라우저 토스트 + 사용량 알림 뿐.  
...
6. **프롬프트 키워드 트리거 (OMC `ultrathink` / `deepsearch`)**  
   세션 내 특정 키워드로 특별 동작 유발. LazyCodex 의 Prompt Library 는 저장·복제만.  
...
7. **OpenClaw Gateway 식 외부 연동 (OMC)**  
   ...
   → LazyCodex 에 **Event Forwarder** 탭 (system 그룹) — codex-code 의 hooks 이벤트 일부를 HTTP POST 로 포워드.
...
9. **Status bar HUD (OMC `omc hud` / OMX `omx hud --watch`)**  
   터미널 상태 바에 실시간 정보. 이미 LazyCodex 는 브라우저 대시보드 자체가 HUD 역할.  
   → 제외 (중복).

10. **Wiki 시스템 (OMX `.omx/wiki`)**  
    세션 내 지식 베이스. LazyCodex 의 Codex Docs Hub + Prompt Library 로 이미 커버됨.  
    → 제외 (중복).

11. **`omx doctor` 진단** (OMX)  
    설치 무결성 검증. LazyCodex 의 aiEval + system 탭이 유사 기능.  
    → 제외.

---

## LazyCodex 가 개선해야 할 최적화 포인트 (OMC/OMX 관찰로부터)

- **세션 내 통합 부재** — OMC/OMX 는 Codex CLI 세션 안에서 슬래시 명령으로 호출. LazyCodex 는 브라우저 전환 필요. → 해결: **MCP 서버 모드** 추가 검토 — `lazycodex` MCP 서버로 등록하면 세션 내에서 `/lazycodex/...` 로 주요 기능 호출.
...
| B7 | Event Forwarder 탭 (Codex CLI hook → HTTP POST) | MED #7 | medium | 중 |
