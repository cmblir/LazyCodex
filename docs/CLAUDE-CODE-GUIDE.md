# 📘 Codex CLI 로 이 대시보드 쓰기 / 확장하기

이 문서는 **Codex CLI 사용자** 가 이 대시보드 템플릿을 자기 환경에서 쓰고, 필요하면 새 카드 / endpoint / 데이터 소스를 직접 추가하는 방법을 설명합니다.
...
```bash
cd ~/Desktop/codex-workspace-dashboard-template
codex
```
...
1. `~/.codex/agents/<your-agent>.md` 만들기
...
def get_my_thing() -> dict:
    """예: ~/.codex/my-data.json 읽어 응답."""
    p = Path.home() / ".codex" / "my-data.json"
    if not p.exists():
        return {"items": []}
    try:
...
- **`server.py` 의 read-only mock 동작 함부로 풀지 말기** — PUT/POST/DELETE 가 진짜로 ~/.codex 를 수정하면 의도치 않은 사고 가능
...
# 2. 모든 endpoint 응답 확인
for ep in codex-md system/status skills agents hooks plugins connectors projects \
...
/plan 새 "최근 방문 프로젝트 Top 10" 카드 추가하려고 해.
      ~/.codex/history.jsonl 에서 가장 자주 등장한 프로젝트 10개를
...
|---|---|---|
| `GET /api/codex-md` | `~/.codex/AGENTS.md` | `{sections, raw}` |
| `GET /api/system/status` | `settings.json` + `sessions/*.json` | `{hooks, permissions, sessions, settings}` |
| `GET /api/skills` | `~/.codex/skills/*` | array |
| `GET /api/agents` | `~/.codex/agents/*.md` | `{agents:[...]}` |
| `GET /api/hooks` | `settings.json hooks` | `{hooks, permissions}` |
| `GET /api/plugins` | `installed_plugins.json` | array |
| `GET /api/connectors` | `~/.codex.json mcpServers` | `{platform, local}` |
| `GET /api/projects` | `~/.codex/projects/*` | `{projects:[...]}` |
...
**Q. 다른 디바이스의 ~/.codex 도 보고 싶다.**
A. Syncthing 으로 `~/.codex` 동기화 + `.stignore` 에서 `/sessions`, `history.jsonl` 제외 풀기. 단 충돌 위험 있음.
