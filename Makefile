...
help:
	...
	@echo "  app                  — build dist/LazyCodex.app (macOS bundle)"
	@echo "  install-mac          — build + copy LazyCodex.app to /Applications/"
	@echo "  uninstall-mac        — rm -rf /Applications/LazyCodex.app"
...
install-mac: app
	@if [ -d /Applications/LazyCodex.app ]; then \
	  echo "▶ replacing existing /Applications/LazyCodex.app"; \
	  rm -rf /Applications/LazyCodex.app; \
	fi
	cp -R dist/LazyCodex.app /Applications/
	@echo "✅ installed to /Applications/LazyCodex.app"
	@echo "   open via Spotlight (⌘Space → 'LazyCodex') or double-click in Finder"

uninstall-mac:
	rm -rf /Applications/LazyCodex.app
	@echo "🗑  removed /Applications/LazyCodex.app"
