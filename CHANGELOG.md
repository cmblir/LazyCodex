...
- The Codex mascot: it is general dashboard chrome (workflow/modal/chat
...
User: "lazyclaw 대시보드에서 쓸 수 있어야해. 지금 lazyclaw에 있는 기능들은 모두."  + "lazycodex랑 lazyclaw 마스코트 모두 변경해줘".

Dashboard now mirrors every surface of the npm `lazyclaw` CLI as its own tab — 16 tabs grouped under a new `🦞 lazyclaw` NAV group. Mascot (both CLI banner and dashboard floating sprite) is rebuilt from the latest Codex Design handoff bundle as a true 16×16 pixel sprite with four states (`idle / working / done / error`).
...
The dashboard's old `#codexMascot` was a draggable orange-square Codex (`mascotJumping` / `mascotSpeaking` modes). It is now rebuilt from the Codex Design handoff (`untitled/project/lazyclaw.html`) as a 16×16 pixel sprite with the Codex-Original palette:
...
### Migration
  ...
  - No DB or `~/.codex-dashboard-*.json` schema change.
...
---
## [3.99.23] — 2026-05-12  🎨 mascot — rebuilt from Codex Design handoff (crab-helmet asterisk star)

User: "터미널이랑 lazyclaw [Codex Design handoff URL] 로고 이거 참고해서 다시 만들어줘."

The bunny mascot from v3.99.16 is retired. The Codex Design handoff bundle (`api.anthropic.com/v1/design/h/.../lazyclaw.html`) ships a proper "lazyclaw" mascot sheet — a 16×16 pixel character: **Codex's asterisk (`*`) star wearing a crustacean helmet with two antenna-claws**, four lifecycle states (idle / working / done / error), four colourway palettes (CLAUDE ORIGINAL / GAME BOY DMG / NES / CORAL CRUSTACEAN), and ASCII fallbacks in two sizes. v3.99.23 adopts the **CLAUDE ORIGINAL** palette + idle pose in both surfaces lazyclaw renders:
...
Colours via truecolor ANSI: helmet body `#c33d2a`, helmet shine `#ff8a6a`, antenna `#a02b1c`, star (Codex orange) `#d97757`, eye slits `#7a6e5f`. Degrades to visible glyphs on terminals that ignore truecolor.
...
The `lazycodex` SPA dist still has the original Codex character mascot (a separate piece of artwork — different brand, different rabbit). That one isn't touched.
...
```
> orchestrator: `cfg.orchestrator` is not set. Defaulting to a single-agent chain on `codex-cli`.
> Configure properly:  `lazyclaw orchestrator set-planner codex-cli` then  `lazyclaw orchestrator workers add <provider:model>` (one per agent).
```
...
$ lazyclaw orchestrator set-planner codex-cli:codex-opus-4-7
$ lazyclaw orchestrator workers add openai:gpt-4o
$ lazyclaw orchestrator workers add gemini:gemini-2.5-pro
$ lazyclaw orchestrator status
{ "planner": "codex-cli:codex-opus-4-7", "workers": ["openai:gpt-4o","gemini:gemini-2.5-pro"], ... }
...
orchestrator: unknown provider "bogus" — registered: gemini, openai, codex-cli, anthropic, ...
```
...
```json
{
  ...
  "orchestrator": {
    "planner": "codex-cli:codex-opus-4-7",
    "workers": [
      "codex-cli:codex-sonnet-4-6",
      "openai:gpt-4o",
...
The lazycodex SPA continues to focus on what it's actually good at: Codex CLI configuration, workflows, providers, agents, observability — none of which was lazyclaw-specific.
...
---
## [3.99.16] — 2026-05-11  🐰 lazycodex mascot lands in lazyclaw · 🛠 dashboard mutation: providers / rates / config edit · 🔍 single-provider live test

User: "dashboard 미구현 항목이 지금 너무 많음. 수정필요. 그리고 lazyclaw 좌측 상단 캐릭터 lazyclaud의 캐릭터로 변경 필요함."

Two threads.

### 1. The lazycodex mascot now wears both hats

The web dashboard's header used to show `🦞 LazyClaw` (lobster emoji + wordmark). The CLI launcher / chat banner had a sleepy ASCII cat (`( -.- )  z z`) on the right side. Both are now the same orange pixel-art rabbit mascot the lazycodex SPA already uses.
...
v3.99.14 added eight built-in OpenAI-compatible vendors and (defensively) added all eight names to `RESERVED_PROVIDER_NAMES`. That made `+ Add a custom OpenAI-compatible endpoint…` reject `nim` with `custom provider name "nim" is reserved (built-in)` — even though the user's whole reason to register a custom `nim` is to point the alias at a different baseUrl/api-key (private NIM proxy, on-prem instance, …). Built-in and custom both go through `makeOpenAICompatProvider`, so a same-name override is well-defined.
  ...
  - `RESERVED_PROVIDER_NAMES` now lists only the providers whose factory is bespoke (`mock`, `codex-cli`, `anthropic`, `openai`, `gemini`, `ollama`) plus the three picker sentinel keywords. The eight OpenAI-compat builtins (`nim`, `openrouter`, `groq`, `together`, `xai`, `deepseek`, `mistral`, `fireworks`) are no longer reserved.
  ...
  - The "real" reserved names (`gemini`, `openai`, `codex-cli`, …) still reject as before — overriding those would silently break the wire format.
...
2. **No way to register NVIDIA NIM / OpenRouter / vLLM / LM Studio
   ...
   `codex-cli`). Anyone wanting a different OpenAI-compatible
...
**Verification**
  ...
  - Playwright 3-viewport visual verification (AGENTS.md §8.4)
...
- Live: `printf '안녕\n/exit\n' | lazyclaw chat` (against the
  user's codex-cli config) outputs `안녕하세요! 무엇을
...
codex 이런식으로 고르고 상세 모델 고를 수 있게 해줘."
...
Step 1 of 3 — pick how you want to auth
  ❯ API key       gemini · openai · anthropic        [needs key]
    CLI / Local   codex-cli · ollama                [no key]
    Mock          offline echo, testing only         [test]

Step 2 of 3 — pick a CLI / Local provider
  ❯ codex-cli    models: opus-4-7 · sonnet-4-6 · …  [no key]
    ollama        models: llama3.1 · qwen3.5 · …     [no key]

Step 3 of 3 — pick a model for codex-cli
  ❯ codex-opus-4-7
    codex-sonnet-4-6
    codex-haiku-4-5
    …
...
- `echo codex-cli | lazyclaw onboard --pick` (non-TTY)
...
>  것들. 그리고 openclaw처럼 대시보드를 띄우는데 lazycodex의
>  lazyclaw 부분만 띄워주면 돼. lazycodex를 전부 띄우기에는
>  너무 무거우니까."
...
The full lazycodex dashboard (Python + dist/index.html ~50
...
looking for:
  ...
  3. codex-cli   (subscription, no key)
  4. anthropic    (API)
  5. ollama       (local)
  6. mock         (test only)

Picker default cursor moved from `codex-cli` to row 0
...
`import('../sandbox.mjs')` (from `providers/codex_cli.mjs`)
...
- Verified end-to-end: `npm pack --dry-run` shows 31 files
  ...
  --non-interactive --provider codex-cli`, `lazyclaw
...
User report: "지금 lazycodex가 다른 pc에서 앱으로 열기하면
...
- **Explicit manifest `id` + unique start_url.**
  ...
  - `"id": "/lazycodex-dashboard"`
  - `"start_url": "/?app=lazycodex"` (the `?app=lazycodex`
...
- **README + AGENTS.md docs** in en / ko / zh updated with the
...
- Live: `lazyclaw skills install anthropics/codex-quickstarts
  @main --max-bytes 67108864` fetches the real tarball,
  installs `codex.md` + `readme.md` as skills, and
...
actually need: confine the `codex` CLI's filesystem and network
...
lazyclaw agent "..." --sandbox docker:alpine \
                     ...
                     --sandbox-mount "$HOME/.codex:/root/.codex:ro" \
                     --sandbox-env CODEX_HOME
```
...
- `--sandbox-mount host:container[:mode]` repeats. Empty by
  default — pass `$HOME/.codex:/root/.codex:ro` to expose the
  user's `codex` login so the sandboxed CLI runs under the
  existing subscription.
- `--sandbox-env <NAME>` repeats; whatever's in the host env
  with that name is passed through to the container.

### Where it applies
Subprocess providers only — currently just `codex-cli`. API
...
- `providers/codex_cli.mjs` swaps its `spawn()` for
...
### Out of scope (decline-by-honesty per AGENTS.md §1)
...
### Subscription mode — `codex-cli` provider
New `src/lazyclaw/providers/codex_cli.mjs`. Spawns the local
`codex` binary with `-p <prompt> --output-format stream-json
...
other provider. Authentication = whatever the `codex` CLI is
...
- `anthropic.mjs` hits api.anthropic.com directly with `sk-ant-`
  keys (pay-per-token).
- `codex_cli.mjs` delegates auth + billing to the existing
  `codex` CLI session (subscription quota).
Both can coexist; users pick at onboard.

The CLI maps full Anthropic model ids back to short aliases
(`codex-opus-4-7` → `opus`) before passing `--model`, because
...
|---|---|
| codex-cli | codex-opus-4-7, codex-sonnet-4-6, codex-haiku-4-5, opus, sonnet, haiku |
| anthropic  | codex-opus-4-7, opus-4-6, sonnet-4-6, sonnet-4-5, haiku-4-5, 3-5-sonnet, 3-5-haiku |
...
### Onboarding UX
- Picker default cursor lands on `codex-cli` so first-time
  users with Codex CLI already installed get the keyless
  default.
- Each row carries an auth label: `[subscription]` (codex-cli),
...
- `cmdDoctor` uses the same `requiresApiKey` flag instead of the
  old `provider !== 'mock'` heuristic, so codex-cli + ollama
...
LAZYCLAW_CONFIG_DIR=/tmp lazyclaw onboard \
  --non-interactive --provider codex-cli --model codex-haiku-4-5
# {"ok":true,"provider":"codex-cli","model":"codex-haiku-4-5","hasApiKey":false}
...
'prerelease')`. The debug log's `cwd /Users/o/lazycodex` made
the cause clear — npm was reading the **root** `package.json`
(`codex-dashboard`, `private: true`, no `version` field) and
...
1. The server emits `event: done` with `ok: false` + `error: "..."`
   when a provider fails (ollama runner crash, codex-cli not on
...
---
## [3.79.0] — 2026-05-06  🔁 template assignee patch consolidation + Codex CLI aliases
...
- The helper additionally accepts the canonical Codex CLI aliases —
  `codex:opus`, `codex:sonnet`, `codex:haiku` — when codex-cli is
  installed, mirroring `server/ai_providers.py::_CLI_FLAG_ALIASES`.
  Previously the helper substituted `codex:sonnet` even on a working
  codex-cli setup because the strict id check looked for
  `codex:codex-sonnet-4-6`.
...
- `dist/app.js`: extracted `_wfPatchTemplateAssignees`, wired into
  ...
  `_WF_CODEX_CLI_ALIASES` constant
...
- Synthetic helper test: `codex:sonnet/opus/haiku` preserved on
  codex-cli-available systems; bogus assignees swapped; multiAssignee
...
- **Quick Action templates (`Autopilot` / `Ralph` / `Ultrawork` /
  `Deep Interview`) hardcoded `codex:sonnet`-class assignees** in
  every session/subagent node. On a machine without codex-cli the
...
- **Commands tab "convert to workflow"** also pinned `codex:sonnet`.
...
### Added
- **`lazycodex validate <wfId|name>`** — preflight check against
  ...
  `lazyclaw validate`.
- **`lazycodex graph <wfId|name>`** — ASCII summary of a workflow's
...
- New verbs run shell-free + emit expected output:
  - `lazycodex validate <wfId>` → "preflight ... ✅ all assignees
    resolve to available providers"
  - `lazycodex graph <wfId>` → "L0: ⏵ S \\ L1: ↦ X \\ L2: ⏎ O"
...
### Fixed
- **`_wfMultiAssigneeAdd` was hardcoded to `codex:opus / codex:sonnet
  ...
  hardcoded. On a machine without codex/openai/gemini installed the
...
- **Run Center modal `_rcExecute` fell back to `'codex:sonnet'`** when
...
- **Crew wizard pre-filled persona models that the user had never
  ...
  `[codex:sonnet, gemini:gemini-2.5-pro, ollama:llama3.1]`. If the
...
- **`/api/workflows/runs` rejected calls without `wfId`.** The endpoint
  required a workflow id, but `lazycodex runs` (added v3.69) and the
...
### Added
- **`lazycodex completion <bash|zsh>`** — print a shell completion
...
- **`lazycodex daemon`** — probe ports 3737 / 3838 for a running
...
- **`lazycodex onboard`** — jump to the onboarding tab. Mirrors the
...
- **Four new terminal verbs**, mirroring more of the `lazyclaw` CLI:
  - `lazycodex providers [filter]` — alias of `keys`.
  - `lazycodex inspect <wfId|name>` — workflow snapshot (nodes/edges/
    tags) + last 5 runs.
  - `lazycodex runs [filter]` — recent workflow runs across all
    workflows.
  - `lazycodex rates` — cost-per-token table for every available
    model.
...
- **`/parallel <a1,a2,...> <task>` chat slash** — fan-out the same
  ...
  Aliased to `/fanout`.
- **Terminal `lazycodex agents|sessions|skills|doctor`** — bring four
  more verbs to parity with the lazyclaw CLI:
  - `agents [filter]` — list registered Codex CLI agents (`/api/agents`)
...
별도 subprocess 로 `codex --resume <id>` 를 spawn 하는
...
```
GET /rates/shape
→ { "anthropic/codex-opus-4-7": { inputPer1M: 0, outputPer1M: 0, ... }, ... }
```
...
GET /providers/test?prompt=hello-from-daemon&model=codex-haiku-4-5
...
$ lazyclaw providers test
{
  ...
  "totalDurationMs": 832,
  "results": [
    { "name": "mock",      "ok": true,  "durationMs": 27,  "model": "codex-opus-4-7", "replyLength": 33 },
    { "name": "anthropic", "ok": true,  "durationMs": 412, "model": "codex-opus-4-7", "replyLength": 17 },
...
GET /rates
→ 200 {
    "anthropic/codex-opus-4-7": { "inputPer1M": 15, "outputPer1M": 75, ... },
...
```
$ lazyclaw rates copy anthropic/codex-opus-4-7 anthropic/codex-opus-4-8
{"ok":true,"src":"anthropic/codex-opus-4-7","dst":"anthropic/codex-opus-4-8",
 "card":{"inputPer1M":15,"outputPer1M":75,"cacheReadPer1M":1.5,"currency":"USD"}}

$ lazyclaw rates copy anthropic/codex-opus-4-7 existing/key
...
$ lazyclaw rates validate
{
  ...
  "rateCount": 4,
  "issues": [
    "key \"anthropic/codex-opus-4-7\": outputPer1M must be a non-negative finite number (got undefined)",
...
$ lazyclaw providers test anthropic --model codex-haiku-4-5-20251001 --prompt "say hi"
...
const cost = costFromUsage(
  { provider: 'anthropic', model: 'codex-opus-4-7', usage: capturedUsage },
  rates,
);
...
- `/model anthropic/codex-opus-4-7` — unified form switches both
...
who wanted a one-stop overview had to read AGENTS.md or hunt through
errors.
...
- `lazyclaw onboard [--non-interactive]` — guided setup. Accepts the
  unified `--model anthropic/codex-opus-4-7` form (provider extracted
  automatically) or the split `--provider anthropic --model codex-opus-4-7`.
...
- `providers/registry.mjs` — re-exports the real provider, adds two
  helpers: `parseProviderModel("anthropic/codex-opus-4-7")` and
...
**QQ217** — `lazycodex refresh` (alias `reload`) terminal verb
...
QQ212 caught the chat-side ones). `lazycodex wh<Tab>` was
silently a no-op even though `lazycodex whoami` was fully
implemented.

Now extended to cover all new verbs:

* `lazycodex {whoami,keys,uptime,workflows,run,cancel,usage}`
  + the `lz` shorthand variants.
* `lazycodex usage 7` / `usage 30` quick presets.
* `lazycodex help workflow` / `help cost` / `help provider`
  for the QQ214 filter form.

### Verified
- `e2e-terminal-tab-suggest-new.mjs` 8/8 ✅ (single-match
  expansions, multi-candidate `lazycodex w<Tab>` listing,
...
**QQ214** — `lazycodex help` now uses the same section-grouped
...
`lazycodex help cost` matches via "cost usage version uptime"
...
**QQ211** — `/uptime` chat slash + `lazycodex uptime`
...
**QQ210** — `lazycodex cancel [runId|wf]` terminal verb
...
* `lazycodex workflows [filter]` — listing with `🟢` live
  marker + `Nn Nr ok|err|running` summary chips.
* `lazycodex run <id|name>` — same exact-id → id-prefix →
  name-substring resolution. Multiple matches → list.
  Unique → POST `/api/workflows/run`, prints workflow + run
  id + `lazycodex open workflows` watch hint.

`KNOWN_VERBS`, did-you-mean candidates, and `lazycodex help`
...
* `lazycodex keys` — providers + `(cli)` / `(api)` chip +
  `key=…mask` / `key=(missing)` for api-type providers.
* `lazycodex usage [N]` — total USD + call count + top-3
  models. Default 7d, integer arg 1-365, out-of-range
  emits `⚠ 범위 밖`.

Also extended the `KNOWN_VERBS` + did-you-mean candidate
lists so `lazycodex kez` Levenshtein-suggests `keys`.

`lazycodex help` lists both new verbs.
...
**QQ198** — `/whoami` chat slash + `lazycodex whoami` terminal
verb (openclaw-style identity introspection). Both surface
Codex CLI login state — email, plan label, organization,
`codex --version` — pulled from the existing
`/api/auth/status` server-side memo. Graceful fallback when not
logged in points the user at `codex auth login`.
...
* Terminal: `lazycodex whoami` + appended to `lazycodex help`.
...
* `e2e-cache-refresh-loop` — Node's first `fetch()` per process
  ...
  fetch init. Also absorbs transient `~/.codex.json` mtime
  invalidations.
...
terminal `lazycodex tabs <filter>` UX. Header shows
`(N/total · "filter")`. No-match toasts "일치하는 탭 없음".
All four list-style verbs (`/agents`, `/sessions`, `/tabs`,
`lazycodex tabs`) now share consistent filter UX.
...
**QQ191** — `lazycodex tabs <filter>` accepts a substring
...
hit 30+) can narrow down: `/agents codex` shows only the 4
Codex variants. Output also caps at 30 with the same overflow
note as QQ188 `/sessions`. No-match case toasts "일치하는
어시니 없음" instead of dumping a 0-result list. Header reflects
filter context: `(4/14 · "codex")`.

`/help` updated.

### Verified
- `e2e-chat-slash-cost-status.mjs` extended from 31 → 34 checks:
  filtered header, non-codex assignees suppressed, no-match
  toast.
...
for every `lazycodex` verb, but Tab couldn't surface most of
...
with the terminal `lazycodex tabs` (QQ142). `/help` lists it,
...
**QQ181** — extended `/go` (chat) and `lazycodex open`
...
### Verified
- `e2e-terminal.mjs` extended from 4 → 7 checks: `lazycodex di`
  → `diag`, `lazycodex ta` → `tabs`, `lazycodex op` returns
  ≥3 candidates.
...
Hitting Tab after `lazycodex di`/`ta`/`op` now expands as
expected.

### Verified
- Manual probe: `lazycodex di` → `lazycodex diag`,
  `lazycodex ta` → `lazycodex tabs`, `lazycodex op` cycles
...
`lazycodex <verb>` terminal built-in. Asserts each verb routes
...
- `e2e-terminal-set-prefs.mjs` extended from 26 → 28 checks:
  `lazycodex open bogusXYZ` doesn't change view, log line
  mentions unknown-tab.

---
## [2.71.59] — 2026-05-03

**QQ169** — `/go bogusXYZ` (chat) and `lazycodex open bogus`
...
`lazycodex tabs` (terminal) when unrecognised. The user stays
on the current tab.

### Verified
- chat: `/go bogusXYZ` → toast "알 수 없는 탭: bogusXYZ — /tabs",
  `state.view` stays `lazyclawChat`.
- terminal: `lazycodex open bogus` → log "⚠ 알 수 없는 탭: bogus
  — lazycodex tabs", `state.view` stays `lazyclawTerm`.
...
Hamming-on-shorter heuristic, so `lazycodex vrsion` couldn't
find `version`. Same fix, same threshold (≤3).

### Verified
- `e2e-terminal-set-prefs.mjs` extended from 24 → 26 checks:
  `lazycodex vrsion → version`, `lazycodex resett → reset`.
...
terminal `lazycodex version` from QQ141). Hits `/api/version`
and posts a LazyCodex info bubble inline. Tab autocomplete,
...
- `e2e-chat-slash-cost-status.mjs` extended to 27 checks (was
  ...
  the LazyCodex header and a version label. All green.

---
## [2.71.40] — 2026-05-03

**QQ150** — `lazycodex diag` terminal builtin. Reuses the
existing `_lcTermHealthCheck` (codex/ollama/gemini/codex/git
...
- `scripts/e2e-terminal-diag.mjs` — Playwright regression: clears
  the log, runs `lazycodex diag`, asserts the health-check
  start/end markers and that each CLI probe (codex/ollama/git)
...
matching the bash convention. Distinct from `lazycodex reset`
...
`lazycodex help`. Stays client-side, never hits the shell.
...
**QQ145** — `lazycodex status` terminal builtin; also restores
the `lazycodex status` autocomplete entry that QQ141 dropped.

The shell whitelist *did* allow `lazycodex status` but
`shutil.which("lazycodex")` returns nothing on most systems, so
running it ended in `lazycodex not installed`. Intercepting it
...
- `e2e-terminal-set-prefs.mjs` extended to 21 checks (was 18) —
  `lazycodex status` prints the header, mentions theme + lang,
  and never hits the shell.
- `e2e-terminal.mjs` re-greens (4/4): the autocomplete suggestion
  for `lazycodex sta` → `lazycodex status` works again.
...
and `api_cli_sessions_list` (full Codex sessions scan) on every
...
**QQ142** — `lazycodex open <tab>` and `lazycodex tabs` terminal
built-ins. Mirrors the chat `/go` alias map so users can jump
between dashboard tabs from the terminal too:

```
lazycodex open wf       # → workflows
lazycodex open chat     # → lazyclawChat
lazycodex open ai       # → aiProviders
lazycodex tabs          # list every NAV tab id with emoji + label
```
...
- `e2e-terminal-set-prefs.mjs` extended to 18 checks (was 16):
  `lazycodex tabs` lists `workflows` + `lazyclawChat`,
  `lazycodex open wf` actually flips `state.view` to
  `workflows`. All green.

---
## [2.71.31] — 2026-05-03

**QQ141** — `lazycodex version` (and `--version` / `-v`) terminal
...
- `e2e-terminal-set-prefs.mjs` extended to 16 checks (was 13):
  `lazycodex version` prints `LazyCodex v2.71.30`, `lz --version`
...
~750ms / ~400ms `<tool> --version` and `codex auth status` costs;
...
`codex --version` *and* `codex auth status` subprocesses
...
- 30s server-side memo for `/api/auth/status` in
  `server/auth.py`. Auto-invalidates when `~/.codex.json`'s
  mtime changes — so `codex auth login` is reflected without
  waiting for the TTL.
...
**QQ121** — bare `lazycodex`, `lz`, `lazycodex --help`, and
...
- `e2e-terminal-set-prefs.mjs` extended to 13 checks
  (was 10) — bare `lazycodex` shows help, `lz --help` shows
...
remember whether they registered `codex:opus`,
...
### Added
- `lazycodex help` (and `lz help`) — terse listing of every
...
- `lazycodex reset` — wipes the terminal log buffer
...
```
lazycodex get                         # full CC_PREFS dump
lazycodex get ui                      # one section
lazycodex get ui.theme                # one key
lazycodex set ui theme light          # bool/int/float/string coerced from schema
...
- Autocomplete suggestions for `lazycodex get/set` and `lz`.
...
`AI 聊天 — 등록된 AI 提供商(Codex·OpenAI·Gemini·Ollama 등)与 직접 대화...`.
...
- `scripts/e2e-find-missing-i18n.mjs` skips `.first-user-prompt`,
  `.prose`, `.prose-codex`, `.markdown`, `pre`/`code`, and the
...
- **Force-bind checkbox** in the Auto-Resume session-detail panel:
  ...
  a new Codex session at the bound cwd.
...
- 🎭 **Chat slash commands regression**
  ...
  4. `/model codex:haiku` flips the dropdown value and
...
- 🎭 **Playwright regression for lazyclaw terminal**
  ...
  3. QQ12 Tab completes `lazycodex sta` → `lazycodex status`.
...
- ✅ **Multimodal extractor regression suite** (QQ72). New
  ...
  helper that QQ40-QQ43 (vision routing), QQ49 / QQ55 (codex-cli
...
- 🏷 **Send button shows current assignee in tooltip** (QQ68). Hover
  the chat ↑ send button to see "전송 (Enter) → codex:opus" or
...
- Multimodal routing for OpenAI / Anthropic / Gemini / Ollama in
  ...
  stripped before codex-cli with a note (QQ49, QQ55); soft warning
...
- 🛡 **Strip base64 images from the help-bot chat prompt too** (QQ55).
  ...
  to `codex -p`. Now scrubbed via the same
...
### Fixed
- 🛡 **Strip base64 images from codex-cli prompts** (QQ49). codex-
...
- ⚠ **Soft vision warning when sending images to non-vision models**
  ...
  `codex-`), the user gets a one-time-per-session toast warning.
...
- ⏱ **Per-command elapsed time in the lazyclaw terminal** (QQ17).
  ...
  invocations (codex cold start, ollama list scan).
...
- 🩺 **Auto health-check on terminal tab open** (QQ4). Once an
  hour, the first visit auto-runs `codex --version`, `ollama list`,
...
- ⌨ **터미널 tab in LazyClaw mode** — whitelisted read-only
  ...
  `codex --version`, `codex config list`, `codex config get <key>`,
  `ollama list`, `ollama ps`, `gemini --version`, `codex --version`,
  `lazycodex status`, `git status`, `git log -5`, `which …`,
...
- ⏱ **Per-workflow node timeout** override (PP4). Slider in
  ...
  need long Codex reasoning (raise to 600 s).
...
### Added
- 🌊 **Streaming chat response** for Codex assignees (OO3).
  `POST /api/lazyclaw/chat/stream` runs `codex-cli` with
...
- 🪝 Non-Codex providers fall through to the one-shot
...
- 💬 **AI 채팅 tab** in LazyClaw mode (OO1). Direct conversation
  with any registered AI provider (Codex / OpenAI / Gemini /
...
### Added
- 📂 **Multi-Codex session reuse picker** in the workflow node
  inspector (CC5). Next to the `session_id 직접 입력` field is a
  `📂 최근` button that opens a modal listing the 30 most recent
  Codex sessions (project, started-ago, first-prompt preview).
...
- 🛑 **Workflow fail-fast: any node fail = whole-run stop** (MM1).
  ...
  (e.g. GPT 401), sibling nodes (Codex / Gemini CLIs) kept hanging
  for their own 60–300s subprocess timeout — the user saw `Codex
  ...
  3. Providers (`CodexCli`, `GeminiCli`, `Codex`) switched from
...
- 🛠 **`node n-fe: all providers failed` actually fixed end-to-end** —
  ...
  even when codex-cli sonnet hangs. Six independent issues stacked on
  top of each other; this release fixes them all (FF1).
  1. **codex-cli `--model codex-sonnet-4-6` deterministically hangs**
     (Anthropic backend). Same call with `--model sonnet` (alias)
     completes in ~30 s. Fix: `CodexCliProvider._resolve_model` now maps
     full model names back to CLI-friendly aliases.
  2. **Subprocess hang on stdin** — added `stdin=subprocess.DEVNULL` so
     codex-cli never waits for tty input even with `-p`.
...
- 🔁 **Default fallback chain extended**: `codex-cli → anthropic-api →
...
- 🔎 Reproduced the user-reported `node n-fe: all providers failed` against
  the saved "팀 개발 스프린트" workflow. Root cause: `codex-cli` and
...
- 🦞 **Mode renamed: OpenClaw → LazyClaw**. Header dropdown label,
  ...
  the project's naming convention (LazyCodex → LazyClaw).
...
### Added
- 🎚️ Top-level mode switcher (All / Codex / Workflow / Providers / LazyClaw).
...
- Per-agent isolated workspace at
  `~/.codex-dashboard-agents/<binding-id>/{AGENTS.md, memory/}`.
...
| # | Where | What |
|---|---|---|
| 1 | `server/backup.py` (new, ~280 lines, stdlib only) | tar.gz archives in `~/.codex-dashboard-backups/` containing all `*.json` data files + a sqlite-vacuumed snapshot via `VACUUM INTO`. Manifest at archive root with `{version, files, createdAt, hostname, label}`. |
| 2 | `api_backup_list` | Returns sorted-by-mtime list with `name, path, sizeBytes, createdAt, files`. |
| 3 | `api_backup_create({label?})` | Generates `lazycodex-YYYYMMDD-HHMMSS[-label].tar.gz`. Atomic `.tmp` + rename. Backs up: `*.db`, all `~/.codex-dashboard-*.json` files (silently skip missing), `~/.codex-code-router/config.json` (flattened to `codex-code-router__config.json`). |
...
| # | Where | What |
|---|---|---|
| 8 | `server/sessions.py::api_sessions_search` | Streams `~/.codex/projects/*/*.jsonl` line-by-line (no whole-file load). Score = occurrences + recency boost (`max(0, 30 - days_old)`). Top-200 most-recent sessions cap, ≤5 matches per session early-termination. In-memory TTL-30s cache (capacity 64). |
...
| 13 | `tests/test_mcp.py` (158 lines) | 13 — `_load_disk_cache` idempotent, `_codex_mcp_list_cached` shape, TTL behavior. Mocks `subprocess.run`. |
...
| 2 | `hyper_advise_auto_resume(entry, recent_failures, assignee="codex:haiku")` | Pre-validates `len(recent_failures) ≥ 2` and entry not in done/stopped. Calls existing `execute_with_assignee` meta-LLM path (Haiku default — fast + cheap). Post-clamps `pollIntervalSec` to [60,1800], `maxAttempts` to [1,50], `promptHint` to 500 chars, `rationale` to 300 chars. |
...
| # | Where | What |
|---|---|---|
| 4 | CSS `body[data-mascot-hidden="true"] #codexMascot` | Already correct. Toggle flips via `_applyPrefsToDOM`. |
...
| 11 | `scripts/ar-haiku-summary.py` (new, executable, 198 lines, stdlib only) | Direct Anthropic Messages API helper bypassing the `codex -p` subprocess. Reads key from `ANTHROPIC_API_KEY` env or `~/.codex-dashboard-ai-providers.json`. POSTs `codex-haiku-4-5-20251001`, max_tokens 200, 10s timeout. Six distinct exit codes (1 missing, 2 no-key, 3 HTTP, 4 network, 5 parse, 6 unexpected). `--dry-run` redacts the API key in headers. Empty stdout on any failure → shell falls back to no-summary mode. |
...
| 5 | `server/system.py / auth.py / toolkits.py` | Unused imports (`MEMORY_DIR`, `re`, `Path`, `CODEX_HOME`, `log`). |
...
7 new indexes added after running `EXPLAIN QUERY PLAN` against the live `~/.codex-dashboard.db` for every static SQL in `server/*.py`. Each query went from `SCAN + TEMP B-TREE` to `SCAN/SEARCH USING INDEX` (no sort step).
...
`~/.codex-dashboard-workflows.json` previously stored both definitions
...
| B12 | `server/learner.py::_collect_sessions` | Replaced `~/.codex/projects/*/*.jsonl` filesystem walk with SQL queries against indexed `sessions + tool_uses` tables. Same return shape; cuts the warmup learner cycle. |
...
| 2 | `server/prefs.py` | New pref `behavior.autoStartOllama` (`bool`, default `True` for back-compat). Validated in `PREFS_SCHEMA["behavior"]`; persisted to `~/.codex-dashboard-prefs.json` like every other behavior key. |
...
via `grep -rn "gemini" server/` + `ps aux | grep gemini` that **lazycodex
...
tool — outside lazycodex's lifecycle. The v2.44.0 Memory Manager tab
...
| 4 | `server/ccr_setup.py::api_ccr_alias_snippet` | Generates a copy-paste block (`# >>> zcodex (lazycodex) >>>` … `# <<<`) with `alias zcodex='ccr code'` and the `eval "$(ccr activate)" && codex` alternative. Detects `$SHELL`, returns the corresponding `~/.zshrc` / `~/.bashrc` path and `already_present` (substring match — read-only). **Never writes to any rc file.** |
...
| 7 | `server/nav_catalog.py` + `dist/index.html` `VIEWS.zcodex` | New tab `🛣️ zcodex (CCR)` under the `config` group. |
...
Source: `https://raw.githubusercontent.com/musistudio/codex-code-router/main/{package.json,README.md}` fetched 2026-04-29.
- npm: `@musistudio/codex-code-router` v2.0.0, bin `ccr`, node ≥20.0.0
- Config: `~/.codex-code-router/config.json`, env interpolation `$VAR` / `${VAR}`
...
| 2 | `dist/index.html` `_wfMultiAssignee*` helpers | `_wfMultiAssigneeRows / Set / Add / Remove / RowHtml`. Stored as `node.data.multiAssignee = ['codex:opus', 'openai:gpt-4.1', …]`. Back-compat: `assignee = rows[0]`; `multiAssignee = rows.length ≥ 2 ? rows : []`, so single-assignee nodes keep behaving exactly as before. |
...
| # | Where | What |
|---|---|---|
| 1 | `server/process_monitor.py` (new) | Stdlib-only module. `lsof -nP -iTCP -sTCP:LISTEN` + `lsof -nP -iUDP` parser; `~/.codex/sessions/*.json` + `os.kill(pid,0)` liveness; macOS `vm_stat` / `sysctl hw.memsize` / `sysctl vm.swapusage` snapshot; `ps -axo` for top-30 RSS with Codex-Code detection. |
...
| 3 | `server/process_monitor.py::api_kill_idle_codex` | Bulk SIGTERM all CLI sessions whose `idle_seconds > thresholdSec` (default 600). Same guards per pid. |
...
| 5 | `server/routes.py` | Registers 6 endpoints: `GET /api/ports/list`, `/api/sessions-monitor/list`, `/api/memory/snapshot`; `POST /api/process/kill`, `/api/sessions-monitor/open-terminal`, `/api/memory/kill-idle-codex`. |
| 6 | `server/nav_catalog.py` + `dist/index.html` `VIEWS.openPorts` / `cliSessions` / `memoryManager` | Three new tabs under `observe`. Style mirrored from `VIEWS.system`. Memory tab shows total/used/free/swap progress bars, "Idle Codex CLI 일괄 종료" button, top-30 RSS table with Codex-Code rows highlighted. |
...
GET /api/usage/project?cwd=$HOME/lazycodex
...
| # | Where | What |
|---|---|---|
| 1 | `server/skills.py::list_skills` | Wrapped with TTL+mtime cache (60 s). Fingerprint stat()s only the top-level `~/.codex/skills/` and `~/.codex/plugins/marketplaces/*` dirs (cheap), so a freshly edited skill invalidates immediately. New `force_refresh` kw. |
...
User: "지금 세팅을 도와주는 것들이 필요해. 예를 들어서, codex MD도 global,
...
`~/.codex/` files; project-scope (`<cwd>/.codex/`) was either read-only
(AGENTS.md) or completely missing (settings, settings.local, skills,
...
| Endpoint | Purpose |
|---|---|
| `GET/PUT /api/project/codex-md` | `<cwd>/AGENTS.md` |
| `GET/PUT /api/project/settings` | `<cwd>/.codex/settings.json` (committed) |
| `GET/PUT /api/project/settings-local` | `<cwd>/.codex/settings.local.json` (gitignored personal overrides) |
| `GET /api/project/skills/list` | list project skills |
| `GET/PUT /api/project/skill` | one skill (creates `.codex/skills/<id>/SKILL.md`) |
...
Every handler resolves `cwd` via `_validate_project_cwd` — must be a real directory under `$HOME` or the call returns `invalid or out-of-home cwd`. Permission rules go through the existing `sanitize_permissions` so an invalid `:*` mid-pattern is auto-fixed exactly like the global `put_settings` path. Path-traversal guard on commands re-resolves the final path and refuses anything outside `<cwd>/.codex/commands/`.
...
- **AGENTS.md tab**: 🌐 Global / 📁 Project toggle + project picker. Save dispatches to the right endpoint.
...
- **Skills tab**: project mode lists `<cwd>/.codex/skills/*` only. ＋ New skill creates a directory + seeded `SKILL.md`. Cards open a project-aware editor.
- **Commands tab**: project mode lists `<cwd>/.codex/commands/**/*.md`. ＋ New command, click-to-edit, delete.
...
```
backend smoke (curl)
  /api/project/codex-md?cwd=$HOME/codex-dashboard   → 200, raw len ≈ 7000
  /api/project/settings?cwd=...                       → 200, exists=false (no .codex/)
  /api/project/settings-local?cwd=...                 → 200, parses real allow rules
  /api/project/codex-md?cwd=/tmp                     → 400 invalid or out-of-home cwd
  PUT round-trip (AGENTS.md / settings / skill)       → all atomic, files written
  path traversal (id="../etc")                        → 400 invalid command id

frontend smoke (Playwright)
  codexmd / settings / skills / commands / hooks     → all render, 0 console errors
...
🖥️ button on the workflow canvas was hard-wired to `codex` regardless of
the node's `assignee`, so a node assigned to `@gemini:gemini-2.5-pro` or
`@ollama:llama3.1` would still launch Codex CLI.

| # | Where | What |
|---|---|---|
| 1 | `server/actions.py::_resolve_provider_cli` (new) | Maps `provider:model` (`codex:opus` / `gemini:gemini-2.5-pro` / `ollama:llama3.1` / `codex:o4-mini` + aliases like `anthropic` / `google` / `openai` / `gpt`) to `{provider, bin, args, model}`. Uses the existing `_which()` 11-path fallback to find each CLI. When the requested CLI isn't installed, it returns codex with a `fallback_reason` so the user gets a warning toast instead of a silent re-route. |
| 2 | `server/actions.py::api_session_spawn` | Now accepts `body.assignee`. For Codex (the only TUI that takes a positional prompt without exiting), the prompt is appended as before. For Gemini / Ollama / Codex, the prompt is printed as a banner (`echo '── Prompt ──'; printf …`) before launching the interactive REPL — passing it as a positional would have caused those CLIs to one-shot and exit. Response now carries `provider` / `cli` / `model` / `fallbackReason`. |
...
#### Verification
```
_resolve_provider_cli('codex:opus')        → codex-cli, /Users/o/.local/bin/codex
...
_resolve_provider_cli('codex:o4-mini')      → codex-cli (codex not installed) + fallback_reason
JS smoke (6 blocks): parses OK
```

#### Compatibility
- Body without `assignee` (e.g. existing chat Spawn buttons elsewhere in the
  app) still routes to Codex — old callers unchanged.
- `codex` flags (`systemPrompt` / `allowedTools` / `--resume`) are only
  appended on the codex-cli path; non-Codex CLIs ignore them.
...
User asked which of `advisor tool` / `codex code routines` / `managed
...
| 4 | `server/routines.py` (new, ~210 LoC) | **Full CRUD over `~/.codex/scheduled-tasks/<name>.yaml`** (the existing tab was listing-only). Tiny line-based YAML extractor (no PyYAML — stdlib only) for `name/description/schedule/command/cwd/enabled`. Run-now endpoint uses `subprocess.run(shell=True, timeout=120)` with a strict cwd-under-`$HOME` guard; rejects anything outside. Stdout/stderr capped at 4 KB per stream. Dry-run mode returns the resolved command + cwd without executing. |
...
UI: all 4 view headers render with 0 console errors:
  🖱️ Computer Use Lab · 🧩 Memory Lab · 🧭 Advisor Lab · 🔁 Codex CLI Routines
```
...
| # | Where | What |
|---|---|---|
| 1 | `server/agent_teams.py` (new, ~280 LoC) | Whitelisted schema for saved teams. Each team is `{id: tm-<hex>, name, description, agents: [{name, scope, cwd, role, task}], createdAt, updatedAt}`. Atomic JSON persistence at `~/.codex-dashboard-agent-teams.json` (env override `CODEX_DASHBOARD_AGENT_TEAMS`). Members reference existing agents — the store doesn't duplicate the agent body, so renaming/deleting an agent reflects immediately on the next list. `_agent_exists()` resolves global / project / builtin / plugin scopes via `get_agent` + filesystem checks; missing members surface as `exists:false` + a per-team `missingCount`. |
| 2 | `server/agent_teams.py` `api_agent_teams_*` | Five routes wired through `routes.py`: `GET /api/agent-teams/list`, `GET /api/agent-teams/get/<tm-id>`, `POST /api/agent-teams/save` (create or update), `POST /api/agent-teams/delete`, `POST /api/agent-teams/spawn`. The spawn route returns one descriptor per existing member (`{name, scope, role, cwd, prompt, codexCmd}`) and a `skipped` array for missing agents. The dashboard either drives `api_session_spawn` per descriptor or surfaces the descriptors as copy-pasteable `codex /agents <name> "<prompt>"` strings. |
...
| 5 | `dist/index.html` Agents tab — Spawn flow | Clicking 🚀 opens a modal listing every member's resolved `codex /agents <name>` invocation in a copy-friendly `<pre>`, plus a Skipped panel for any member whose underlying agent file is gone. |
...
- Backend additions only. Existing `/api/project/detail` shape strictly
  extends — every prior key (`cwd`, `name`, `repo`, `codexJsonEntry`,
...
- Teams store is brand-new (`~/.codex-dashboard-agent-teams.json`); first
  write creates it.
...
### 🏷️ Hook names — surface the same identity Codex CLI's `/hooks` shows
...
| 2 | `server/hooks.py::get_hooks` | Same propagation for **user** hooks in `~/.codex/settings.json` — they too can carry `id` / `name` / `description` at the group level. |
...
**(A) Hyper Agent now works on project-scoped sub-agents** (`<cwd>/.codex/agents/<name>.md`).
...
| 3 | `server/hyper_agent_worker.py` | After-session trigger now restricts the jsonl scan to the agent's project transcript dir (`~/.codex/projects/<slug>/`, where slug is `<cwd>` with `/` → `-`). A project-scoped agent no longer fires from chatter in unrelated projects. The worker's per-tick loop iterates by composite key and parses scope/name out of it. |
...
| # | Where | What |
|---|---|---|
| 1 | `server/hyper_agent.py` (new, ~530 LoC) | Whitelisted schema, strict per-key validation, atomic JSON persistence at `~/.codex-dashboard-hyper-agents.json`. `apply_proposal()` writes `~/.codex/agents/<name>.md` after copying the prior content to `<name>.<ts>.bak.md`. `refine_agent()` calls `execute_with_assignee()` against a hardened meta-system-prompt that returns a strict JSON proposal `{newSystemPrompt, newTools, newDescription, rationale, scoreBefore, scoreAfter}`. `rollback()` restores from any backup snapshot and is itself reversible. |
...
| 8 | `scripts/e2e-hyper-agent.mjs` (new) | Playwright smoke: seeds a real `~/.codex/agents/hyper-test.md`, then verifies UI globals · list · configure round-trip · get reflects state · toggle off · modal renders saved objective · refine endpoint responds gracefully. **All 7 checks pass.** |
...
- New file `~/.codex-dashboard-hyper-agents.json` is created on first configure.
- `~/.codex/agents/<name>.md` is read/written using the same Codex CLI-compatible frontmatter shape (`name`, `description`, `model`, `tools`).
- Override the meta path with env `CODEX_DASHBOARD_HYPER_AGENTS=/some/path`.
...
parameter. Values persist server-side at `~/.codex-dashboard-prefs.json`,
...
#### Migration / compatibility
- New file `~/.codex-dashboard-prefs.json` is created on first write — defaults are derived from `DEFAULT_PREFS` until then.
...
- Override the path with env `CODEX_DASHBOARD_PREFS=/some/path`.
...
| 2 | `server/auto_resume_hooks.py` | New `SNAPSHOT_SH_BODY_HAIKU` template + `install(cwd, *, use_haiku_summary=True)` flag. Stop-hook now optionally pipes the jsonl tail through `codex --print --model haiku-4.5 --bare` for a tight ≤12-bullet "where you left off" markdown brief — falls back to raw tail if Haiku unavailable. |
...
### ✨ Auto-Resume — inject a self-healing retry loop into a live Codex CLI session
...
by a token / rate-limit, the worker spawns `codex --resume <id> -p "<prompt>"`
...
| 3 | **Stop-hook progress snapshot** — every Codex response writes `<cwd>/.codex/auto-resume/snapshot.md` so we always have the latest state on disk | `server/auto_resume_hooks.py::install` |
| 4 | **SessionStart-hook injection** — resumed session gets the snapshot piped into context automatically (Codex CLI's SessionStart-hook stdout contract) | `server/auto_resume_hooks.py::install` |
...
| 7 | **Observable state file** — `~/.codex-dashboard-auto-resume.json` with `running`/`waiting`/`watching`/`done`/`failed`/`exhausted`/`stopped`/`error` state per session | `server/auto_resume.py::_dump_all` |
...
- Hook installation is project-local only; `~/.codex/settings.json` (global) is **never** touched
- Settings.json backup written to `<cwd>/.codex/settings.json.auto-resume.bak` before first mutation
...
2. **"ECC 설치했는데 런센터에 안나와."** — After installing ECC from Guide & Tools, Run Center still showed 0 ECC items. Root cause: I scanned only `~/.codex/plugins/cache/ecc/ecc/` but `toolkits.py` installs as `everything-codex-code@everything-codex-code`, which lives at `~/.codex/plugins/cache/everything-codex-code/everything-codex-code/`. **My initial "181 + 79" verification was on my own machine where both ids happened to coexist; I didn't recognise the path-name dependency until the user surfaced it.**

#### Fixes

**Run Center backend (`server/run_center.py`)**
- Replaced single-root resolution with `_ecc_roots()` returning every detected install: (1) `installed_plugins.json` entries for `ecc@ecc` **and** `everything-codex-code@everything-codex-code` (authoritative), (2) cache glob over both package names, (3) marketplaces fallback. Items are deduped across roots.
...
- Two new toolkit cards: `oh-my-codexcode` (OMC) and `oh-my-codex` (OMX). Each card explains:
  - What's already absorbed by LazyCodex (no install needed).
  ...
  - Which features are in LazyCodex only vs CLI only.
...
- Live HTTP — `/api/run/catalog` returns 270 items (262 ECC + 4 OMC + 4 OMX) on a machine with both `ecc@ecc` and `everything-codex-code@everything-codex-code` installed; debug blob lists 4 roots with per-root counts.
- `/api/guide/toolkit` returns 5 toolkit cards (was 3): everything-codex-code, codex-code-best-practice, **oh-my-codexcode**, **oh-my-codex**, wikidocs-codex-code-guide.

#### What I got wrong in v2.36.0

The "181 skills + 79 slash commands" headline was true on my dev box. I should have flagged that it depends on which ECC plugin id was installed, and I should have run `_ecc_root()` against an environment that only had the `everything-codex-code` id (which is what most users get from the dashboard installer). Future audits will trace `installed_plugins.json` first, not glob the cache directly.
...
dashboard rather than only inside Codex CLI sessions. Three additions
land here, all wired to the existing `execute_with_assignee` pipeline so
any provider (Codex / OpenAI / Gemini / Ollama) can serve the request.
...
- **ECC** — 181 skills + 79 slash commands, scanned from
  `~/.codex/plugins/cache/ecc/<version>/{skills,commands}/`. Every
...
- Click a card → modal with goal input, model picker (uses existing
  `_wfAssigneeOptions` so Codex / GPT / Gemini / Ollama appear),
...
- `POST /api/run/favorite/toggle` — persisted to
  `~/.codex-dashboard-run-favorites.json`.
...
- An `ECC` chip when the command's path is under
  `~/.codex/plugins/cache/ecc/` (heuristic — also matches `scope ===
...
### 📦 Install LazyCodex as a real app (PWA + macOS .app bundle)

LazyCodex can now be installed as an app, both ways:

#### Option A — PWA (cross-platform: macOS / Windows / Linux / iOS / Android)

Open LazyCodex in any modern browser → click the install icon in the
...
```bash
make install-mac     # builds + copies LazyCodex.app to /Applications/
```

Double-click in Finder, or open via Spotlight (`⌘Space → LazyCodex`).
The launcher:

1. Resolves the project directory: `$LAZYCODEX_HOME` env > `~/Lazycodex`
   > `~/lazycodex`. Shows a friendly dialog if none found.
...
4. Otherwise starts `python3 server.py` in the background, logging to
   `~/Library/Logs/LazyCodex/server.log`.
...
Node runtime. It depends on the system `python3`, matching LazyCodex's
...
| `make app` | build `dist/LazyCodex.app` |
| `make install-mac` | build + copy to `/Applications/` (replaces existing) |
| `make uninstall-mac` | remove `/Applications/LazyCodex.app` |
...
- `.app` launcher — runtime simulation: server starts, `/api/version`
  responds, log file created at `~/Library/Logs/LazyCodex/server.log`,
...
3. `#codexMascot` (z-index 1150), `#chatBubble`, `#chatLauncher` floated
...
- Floating `#codexMascot` / `#chatBubble` / `#chatLauncher` force-hidden
...
- `server/slack_api.py` — Slack Web API client (Bot Token `xoxb-*`):
  ...
  `auth.test`. Token saved to `~/.codex-dashboard-slack.json` with
  chmod 600.
...
- `bt-crew` (Persona Crew) — Planner (Opus) → 3 personas (Codex / Gemini
...
기존 v2.33.1 의 ECC 자동화는 marketplace 클론까지만 자동이고 실제 플러그인 설치는 사용자가 Codex CLI 에서 `/plugin install` 을 직접 실행해야 했다. `codex plugin install` 서브커맨드가 비대화형으로 동작하는 것을 확인 후 두 신규 엔드포인트 추가:

- `POST /api/toolkit/ecc/install-plugin` — marketplace 미클론이면 선행 클론 후 `codex plugin install everything-codex-code@everything-codex-code -s user` 실행
- `POST /api/toolkit/ecc/uninstall-plugin` — `codex plugin uninstall ...` 실행

GuideHub 카드 UI 가 설치 상태에 따라 "🔌 플러그인 설치" / "🗑 플러그인 제거" 버튼으로 자동 토글.

**검증**
- Playwright end-to-end: "플러그인 설치" 클릭 → 30초 내 "1 개 플러그인 설치됨" 으로 전환 확인
- `codex plugin list` 에 `everything-codex-code@everything-codex-code` v1.10.0 등록 확인

**파일**
- `server/toolkits.py`: `_run_codex_plugin` + 2 신규 API
...
2. **관리 기능 부재** — Everything Codex CLI(ECC) · Codex CLI Best Practice(CCB) 가 카탈로그엔 있지만 설치/제거 버튼이 없어 사용자가 직접 터미널/CC 에서 수행해야 했음 (RTK 는 이미 자동화되어 있음).
...
- **신규 `server/toolkits.py`** — `/api/toolkit/{status,ecc/install,ecc/uninstall,ccb/install,ccb/uninstall,ccb/open}` 6 라우트:
  - **ECC**: `git clone --depth 1 affaan-m/everything-codex-code` → `~/.codex/plugins/marketplaces/everything-codex-code/` + `known_marketplaces.json` 엔트리 등록. 이후 Codex CLI 에서 `/plugin install ...` 만 실행하면 됨(명령 복사 버튼 제공). 제거 시 디렉터리 + 레지스트리 entry 모두 삭제.
  - **CCB**: `git clone --depth 1 shanraisshan/codex-code-best-practice` → 기본 `~/codex-code-best-practice/`. 제거 시 디렉터리 삭제(.git 존재 확인). macOS `open` 으로 폴더 열기 지원.
...
**검증**
- `curl -X POST /api/toolkit/ecc/install` → `{"ok":true,"installed":true}` · `~/.codex/plugins/marketplaces/everything-codex-code/` clone 완료 · `known_marketplaces.json` 에 entry 추가 확인.
...
### 🔌 MCP 서버 모드 — LazyCodex 를 Codex CLI 에서 직접 호출

Codex CLI 세션 안에서 대시보드 기능을 MCP tool 로 호출할 수 있게 LazyCodex 를 stdio MCP 서버로 노출.

**신규 파일**
- `server/mcp_server.py` — stdio JSON-RPC 2.0 루프 (MCP 2024-11-05 protocol)
- `scripts/lazycodex_mcp.py` — 진입점 스크립트

**노출 tools (6)**
- `lazycodex_tabs` — 6 상위 카테고리별 탭 카탈로그
- `lazycodex_cost_summary` — 비용 타임라인 요약 (총 USD · 소스별 · 모델별)
- `lazycodex_security_scan` — `~/.codex` 정적 보안 검사 결과
- `lazycodex_learner_patterns` — 세션 반복 패턴 + 누적 토큰
- `lazycodex_rtk_status` — RTK 설치/훅 상태
- `lazycodex_workflow_templates` — 10 빌트인 템플릿 목록
...
**설치**
```bash
codex mcp add lazycodex -- python3 /<path>/LazyCodex/scripts/lazycodex_mcp.py
```

**대시보드 UI**
- MCP 탭 상단에 "💤 LazyCodex 자체를 MCP 서버로" 카드 추가
...
사용자 요청: ECC(everything-codex-code) 의 좋은 기능 흡수. 가장 가치 높은 **AgentShield** 를 우리 대시보드 형태로 재구현.

**신규 모듈 `server/security_scan.py` — 로컬 휴리스틱, AI 호출 없음**
- 스캔 대상: `~/.codex/settings.json` · `~/.codex/AGENTS.md` · hooks · agents · `~/.codex/mcp.json`
- 이슈 카테고리 (severity: critical/high/medium/low/info):
  ...
  - **tokens**: autocompact threshold 미설정, AGENTS.md 50KB+ (토큰 낭비)
...
Codex CLI 세션 JSONL 에서 반복되는 tool 시퀀스 · 프롬프트를 자동 감지해 Prompt Library / 워크플로우 템플릿으로 저장하도록 제안하는 탭.
...
- 허용값: `""`(none) · `codex-api` · `openai-api` · `gemini-api` · `ollama-api`
...
- Codex CLI hook 이벤트(`PostToolUse`, `Stop`, `SessionStart` 등 9종)를 외부 HTTP endpoint 로 포워딩
- 신규 모듈 `server/event_forwarder.py`:
  - `~/.codex/settings.json` 의 hooks 섹션에 `curl -sS -X POST --data-binary @- '<URL>' # __lazycodex_forwarder__` 엔트리 add/remove
  - 매 변경 시 `settings.json.bak.<ts>` 자동 백업
  - `__lazycodex_forwarder__` 마커로 우리 엔트리만 필터 — 사용자가 직접 추가한 hook 은 건드리지 않음
...
- Policy fallback sanitize 경계 케이스 3종 (정상 / evil / codex-api)
...
**재분류 (6 → 6, 의미 재정비)**
- `new` → **Learn** 🆕 — 신기능 · 온보딩 · Codex Docs · 가이드 (4)
...
사용자 요청: `oh-my-codexcode` (OMC) / `oh-my-codex` (OMX) 분석 후 LazyCodex 에 없는 기능 중 low 위험도 항목을 자율모드로 일괄 반영.  
...
- 신규 모듈 `server/session_replay.py` — `~/.codex/projects/**/*.jsonl` 스캔
...
**분석 문서 (기록)**
- `docs/plans/analysis-omc-omx-gap.md` — OMC/OMX vs LazyCodex 매트릭스 + 흡수 후보 HIGH/MED/LOW 분류 + 추가 최적화 포인트
...
Codex 토큰 **60-90% 절감** 하는 Rust CLI 프록시 [`rtk-ai/rtk`](https://github.com/rtk-ai/rtk) 를 대시보드에서 바로 설치·활성화·통계 조회 가능한 탭으로 통합.
...
- Codex CLI settings.json 내 `rtk` 참조 탐지로 훅 활성 상태 체크
...
**LazyCodex 마스코트 (`docs/logo/mascot.svg`)**
...
**기타**
- `dist/index.html`: 잔여 `Codex Control Center` 4곳 (title · 사이드바 · 온보딩 모달) → `LazyCodex`
...
- overview 탭의 Codex 계정 온보딩 모달을 `Continue|계속|继续` 버튼 자동 클릭으로 통과
- 워크플로우 시드 선택 fallback: `_wfOpen` → `_wfSelect` → 직접 `__wf.current` 할당

**`dist/index.html` UI 브랜드 텍스트 정리**
- `<title>` · 사이드바 브랜드 · 계정 온보딩 모달 헤더 등 4곳의 `Codex Control Center` → `LazyCodex` 치환 (리브랜딩 일관성)
...
### 🎨 Branding — 프로젝트 이름을 **LazyCodex** 로

레퍼런스: [`Yeachan-Heo/oh-my-codexcode`](https://github.com/Yeachan-Heo/oh-my-codexcode) 의 README 스타일 참고.

- **브랜드 네임**: `Codex Control Center` → `💤 LazyCodex`
  - 네이밍 톤: `lazygit` / `lazydocker` / `lazyvim` 패밀리 편승. 게으른 사람을 위한 로컬 Codex 커맨드 센터.
...
- **README 3종 (ko/en/zh)**
  ...
  - ASCII 배너 내 `🧭 Codex Control Center` → `💤 LazyCodex`.
...
- **기술 경로 유지** (하위 호환):
  - Repo URL `github.com/cmblir/codex-dashboard` 유지 (rename 은 사용자 선택 사항).
  - 데이터 파일 `~/.codex-dashboard-*.json` 경로 유지 — 기존 사용자 데이터 보존.
  - 내부 변수명·모듈명 변경 없음.
...
- `codexDocs` · `promptLibrary` · `projectAgents` · `mcp`
...
B6 (Codex.ai Artifacts 의 로컬 대안) 을 안전하게 구현하기 위한 **설계 선행**. 실제 코드는 다음 세션 (T20~T23, v2.22~v2.24) 에 나누어 진행.

**저장 위치**: `Obsidian/Projects/codex-dashboard/decisions/2026-04-23-artifacts-design.md`
...
Codex API 플레이그라운드 10종 + 워크플로우 실행 비용을 **한 화면**에 통합.
...
**Architecture**
- `server/cost_timeline.py` 신설 — 각 `~/.codex-dashboard-*.json` 히스토리를 통합 집계
- 처리 소스 (10 + workflows):
...
**설정**
- `~/.codex-dashboard-batch-budget.json` — `{enabled, maxPerBatchUsd, maxPerBatchTokens}`
...
- 저장: `~/.codex-dashboard-prompt-library.json`
...
T5~T8 (v2.11.0 ~ v2.14.0) 신규 4 탭(serverTools · codexDocs · citationsLab · agentSdkScaffold) 이 README 본문에 반영되도록 일괄 갱신.

- 배지 v2.9.1 → **v2.14.1**
- 미리보기 "45 탭" → **"49 탭"**
- Why 비교표 "45 tabs" → "49 tabs"
- Codex CLI Integration 테이블:
  * 🆕 그룹에 `codexDocs` 추가
...
`codex-agent-sdk` 기반 Python / TypeScript 프로젝트 뼈대를 UI 에서 생성 + Terminal 새 창에 초기화 명령 자동 붙여넣기.
...
- 히스토리 최근 20건 (`~/.codex-dashboard-citations-lab.json`)
...
### 📖 Codex Docs Hub — 신규 탭 `codexDocs` (new 그룹)

docs.anthropic.com 의 주요 페이지를 대시보드 안에서 카테고리별 카드로 색인 + 검색.

**카테고리 5**
- **Codex CLI** — Overview / Sub-agents / Skills / Hooks / MCP / Plugins / Output Styles / Status Line / Slash Commands / Memory / Interactive / IAM / Settings / Troubleshooting (14)
- **Codex API** — Messages / Prompt Caching / Extended Thinking / Tool Use / Message Batches / Files / Vision / Citations / Web Search Tool / Code Execution Tool / Embeddings (11)
...
**Architecture**
- `server/codex_docs.py` 신설 — 정적 `CATALOG` dict + `api_codex_docs_{list,search}`
- `server/routes.py` 2 라우트 (GET list/search)
- `server/nav_catalog.py` `codexDocs` 탭 등록 (`new` 그룹) + en/zh
- `dist/index.html` NAV (icon 📖) + `VIEWS.codexDocs` · `cdSet` · `cdRender` (debounce)
...
### 🧰 Codex 공식 내장 Tools 플레이그라운드 — 신규 탭 `serverTools`
...
- 응답 content 블록 분류 시각화:
  ...
  * `text` — 최종 응답
- 히스토리 최근 20건 (`~/.codex-dashboard-server-tools.json`)
...
- `🤝 Codex CLI Integration` 탭 테이블의 work 그룹에 신규 7 탭(`promptCache` `thinkingLab` `toolUseLab` `batchJobs` `apiFiles` `visionLab` `modelBench`) 🆕 표시로 추가 + "Codex API 플레이그라운드" 하이라이트 줄 추가
...
2026-04-23 연속 릴리스로 **Codex API 플레이그라운드 7 탭**을 work 그룹에 추가: `promptCache`(v2.3.0) · `thinkingLab`(v2.4.0) · `toolUseLab`(v2.5.0) · `batchJobs`(v2.6.0) · `apiFiles`(v2.7.0) · `visionLab`(v2.8.0) · `modelBench`(v2.9.0). 원격 v2.2.1 위에 rebase 된 결과 버전 번호를 한 칸 shift 했다.
...
- 히스토리 `~/.codex-dashboard-tool-use-lab.json`
...
Codex Opus 4.7 / Sonnet 4.6 의 Extended Thinking 을 실험하고 thinking block 을 분리 시각화.
...
- 히스토리 최근 20건 (`~/.codex-dashboard-prompt-cache.json`)
...
**프로바이더 CLI 감지 견고화**
- `server/ai_providers.py` 에 `_which()` 헬퍼 신설 — `shutil.which` PATH 탐지 실패 시 `/opt/homebrew/bin`·`~/.local/bin`·nvm/asdf node 버전 디렉터리 등 **11개 fallback 경로** 전수 검색. LaunchAgent·GUI 런치 등 PATH 가 좁혀진 환경에서도 Codex/Codex/Gemini/Ollama CLI 를 정확히 감지.
- `CodexCliProvider._bin`, `OllamaProvider._bin`, `GeminiCliProvider._bin`, `CodexCliProvider._bin`, `CustomCLIProvider._bin`, 임베딩 실행 경로 모두 `_which()` 로 교체.

**CLI 설치·로그인 원클릭 (신규)**
- 신규 모듈 `server/cli_tools.py` — 4종 CLI 설치·상태·로그인 통합 관리.
  - `GET /api/cli/status` — 4종(codex/codex/gemini/ollama) 설치 여부·버전·경로 + brew/npm 가용성 반환
  - `POST /api/cli/install` — brew 우선 → npm → 설치 스크립트 자동 선택, AppleScript 로 Terminal 열어 **대화형 설치 수행**
  - `POST /api/cli/login` — `codex auth login` / `codex login` / `gemini` 최초 실행 등을 터미널에서 실행
- 설치 방법 카탈로그: `brew install --cask codex-code` · `npm install -g @openai/codex` · `npm install -g @google/gemini-cli` · `curl ... ollama.com/install.sh`
...
- **8개 AI 프로바이더** + 커스텀 무제한: Codex CLI, Ollama, Gemini CLI, Codex + OpenAI API, Gemini API, Anthropic API, Ollama API
...
- 🔄 **백엔드 에러 i18n 완전 전환** — agents, skills, hooks, mcp, plugins, projects, features, commands, codex_md, actions 모듈 29개 에러에 `error_key` 추가

### Architecture
- `server/workflows.py` — `api_workflow_webhook()` Webhook 트리거
- `server/ai_keys.py` — `api_provider_health()` 병렬 헬스체크
- `server/agents.py`, `skills.py`, `hooks.py`, `mcp.py`, `plugins.py`, `projects.py`, `features.py`, `commands.py`, `codex_md.py`, `actions.py` — `err()` / `error_key` 전환
...
- 📊 **실행 이력 강화** — run 항목에 프로바이더별 색상 태그 + 집계 ("Codex ×3, GPT ×1"), duration 읽기 좋은 형태
...
- 🧠 **AI 프로바이더 탭 (aiProviders)** — 멀티 AI 오케스트라 기반 구축
  - **8개 빌트인 프로바이더**: Codex CLI, Ollama, Gemini CLI, Codex (CLI) + OpenAI API, Gemini API, Anthropic API, Ollama API
  - CLI 자동 감지 (로컬 설치된 codex/ollama/gemini/codex) + API 키 설정
  - **커스텀 CLI 프로바이더** — 임의의 CLI 도구를 AI 프로바이더로 등록
  - **폴백 체인** — 1차 프로바이더 실패 시 대안 자동 전환
  - 연결 테스트 + 모델 카탈로그 + 가격표 내장
- 🔀 **워크플로우 멀티 프로바이더 통합**
  - 노드 assignee: `codex:opus`, `openai:gpt-4.1`, `gemini:2.5-pro`, `ollama:llama3.1`, `codex:o4-mini`
  - 기존 Codex 전용 assignee 완전 호환 유지
...
- `server/ai_keys.py` (신규) — `~/.codex-dashboard-ai-providers.json` 설정 CRUD
...
- 🚀 **시작하기 (onboarding)** — ~/.codex 상태 실시간 감지 단계별 체크리스트
